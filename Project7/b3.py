...
def GetTopLevelInstance():
    ...
    for WinID in G_TopLevelDict.keys():
        if G_TopLevelDict[WinID][0] == False:
            ...
            # 设置Toplevel窗口的最小尺寸和最大尺寸为None，去除限制
            G_TopLevelDict[WinID][1].minsize(0, 0)
            G_TopLevelDict[WinID][1].maxsize(None, None)
            G_TopLevelDict[WinID][1].overrideredirect(False)
            G_TopLevelDict[WinID][1].resizable(True, True)
...
def OutputProcessToText(cmdText, uiName, elementName):
    ...
    try:
        import subprocess
        process = subprocess.Popen(cmdText, shell=True, bufsize=0, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
...
def EventFunction_Adaptor(fun, **params):
    ...
    def check_focus_out(event, **params):
        global G_IsMessageBoxState
        EventType = str(event.type)
        if EventType == "10":
            if G_IsMessageBoxState:
                return
        fun(event, **params)

    return lambda event, fun=fun, params=params: check_focus_out(event, **params)
...
def EventTwoFunction_Adaptor(fun1, fun2, **params):
    ...
    if 'threadings' in argspec.args:
        ...
        if threadingvalue != 0:
            ...
            return lambda event, fun=fun_threading, params=params: fun(event, **params)
    try:
        return lambda event, fun1=fun1, fun2=fun2, params=params: (
                    fun1(event, **params) is True and fun2(event, **params))
    except:
        return
...
def ListViewHeadingFunction_Adaptor(fun, uiName, widgetName, columnname):
    ...
    argspec = inspect.getfullargspec(fun)
    if G_PrintFunctionMode == True:
        ParamText = ParamText + "uiName =" + str(uiName) + "," + "widgetName =" + str(
            widgetName) + "," + "columnname =" + str(columnname)
        print('函数调用打印' + fun.__name__ + "(" + ParamText + ")")
...
def SetControlAcceptDrop(uiName, elementName, functionCallback):
    ...
    import windnd
    windnd.hook_dropfiles(Control,
                          func=DropFileFunction_Adaptor(functionCallback, uiName=uiName, widgetName=elementName))
    ...
    try:
        ...
        if isDropTitle == True:
            ...
            from win32con import NULL, SWP_NOSIZE, SWP_NOMOVE, SWP_NOZORDER, SWP_DRAWFRAME, GWL_STYLE, WS_CAPTION, \
                WM_SYSCOMMAND, SC_MOVE, HTCAPTION, WS_THICKFRAME
            WindowHandle = ctypes.windll.user32.GetParent(root.winfo_id())
...
def PlayDestroyDialogAction(uiName, result, topLevel, animation='zoomout'):
    ...
    def ZoomOut(topLevel, zoom, win_x, win_y, win_width, win_height):
        ...
        if zoom > 0.0:
            topLevel.after(1,
                           lambda: ZoomOut(topLevel=topLevel, zoom=zoom, win_x=win_x, win_y=win_y, win_width=win_width,
    ...
    if animation == "fadeout":
        ...
        try:
            ...
            topLevel.after(1,
                           lambda: ZoomOut(topLevel=topLevel, zoom=1.0, win_x=win_x, win_y=win_y, win_width=win_width,
                                           win_height=win_height))
        except ImportError:
            pass
    ...
    if uiName in G_UIElementDictionary:
        ...
        if root is not None:
            if G_TopDialog is root:
                G_TopDialog = None
            for WindowInstance in G_WindowDraggable:
                if root is WindowInstance.GetWidget():
                    G_WindowDraggable.remove(WindowInstance)
                    break
                ...
                if root.master != None or result == 0:
                    ...
                    if IsTopLevel == False:
                        ...
                        if Form1:
                            ...
                            if UIClass:
                                Temp_isTKroot = UIClass.isTKroot
                                UIClass.isTKroot = False
                                if hasattr(UIClass, "Exit") == True:
                                    UIClass.Exit()
                                UIClass.isTKroot = Temp_isTKroot
            except:
                pass
                # UIClass = GetElement(uiName,"UIClass")
        ReleaseEmbedUI(uiName)
        CleanUIDictionaryInfo(uiName)
        # if IsTopLevel == True:
...
def ActiveFrameChildsElement_InEditor(uiName, element):
    ...
    for child in children:
        ...
        if uiName2 and elementName:
            ...
            if uiName2 in G_UIElementAliasDictionary.keys() and realElementName in G_UIElementAliasDictionary[
                uiName2].keys():
...
def CreateRadioButton(uiName, parentName='Form_1', elementName='', groupName='', defaultValue=1, style='indicatoron'):
    ...
    if uiName in G_UIElementDictionary:
        ...
        if ParentElement:
            ...
            newRadioButton = tkinter.Radiobutton(ParentElement, variable=groupVariable, value=defaultValue,
                                                 text="RadioButton", anchor=tkinter.W)
...
def CreateCheckButton(uiName, parentName='Form_1', elementName='', defaultValue=False, style='indicatoron'):
    ...
    if uiName in G_UIElementDictionary:
        ...
        if ParentElement:
            ...
            newCheckButton = tkinter.Checkbutton(ParentElement, variable=checkButtonVariable, text="CheckButton",
                                                 anchor=tkinter.W)
    ...
    if Control and callbackFunction:
        ...
        if eventName == 'Command':
            if RealElementName.find('Scale_') >= 0:
                Control.configure(
                    command=SetValueChangedFunction(callbackFunction, uiName=uiName, widgetName=elementName))
            else:
                Control.configure(command=partial(CommandFunction_Adaptor, fun=callbackFunction, uiName=uiName,
            ...
            Control.bind('<Button-1>',
                         EventFunction_Adaptor(OnListViewCellClicked, uiName=uiName, widgetName=elementName,
                                               callbackFunc=callbackFunction), add=True)
            ...
            for columnName in Columns:
                Control.heading(columnName,
                                command=partial(ListViewHeadingFunction_Adaptor, fun=callbackFunction, uiName=uiName,
                                                widgetName=elementName, columnname=columnName))
        else:
            bindEventName = str('<' + eventName + '>')
            Control.bind(bindEventName, EventFunction_Adaptor(callbackFunction, uiName=uiName, widgetName=elementName))


def SetElementScrollbar(uiName, elementName, orient=tkinter.VERTICAL, ParentTopCanvas=None, ParentBottmCanvas=None):
    ...
    if Control:
        ...
        ParentInfo = ScrollTarget.winfo_parent()
        ParentCanvas = ScrollTarget._nametowidget(ParentInfo)
        if elementName.find('Frame_') >= 0:
            ParentInfo = ParentCanvas.winfo_parent()
            ParentCanvas = ParentCanvas._nametowidget(ParentInfo)
        if orient == tkinter.VERTICAL:
            if ParentTopCanvas:
                VerticleScrollBar = tkinter.ttk.Scrollbar(ParentTopCanvas, orient=tkinter.VERTICAL)
            else:
                VerticleScrollBar = tkinter.ttk.Scrollbar(ParentCanvas, orient=tkinter.VERTICAL)
            ...
            if ParentBottmCanvas:
                HorizontalScrollBar = tkinter.ttk.Scrollbar(ParentBottmCanvas, orient=tkinter.HORIZONTAL)
            else:
                HorizontalScrollBar = tkinter.ttk.Scrollbar(ParentCanvas, orient=tkinter.HORIZONTAL)
        ...
        def ResetVerticleScrollBar(event):
            Control_Width = ParentCanvas.winfo_width()
            Control_Height = ParentCanvas.winfo_height()
            if event.width == 0:
                event.width = Control_Width
            if event.height == 0:
                event.height = Control_Height
            if event.width == 1 and event.height == 1:
                event.width = Control_Width
                event.height = Control_Height
            ScrollTarget = GetElement(uiName, elementName)
            HorizontalScrollBarName = elementName + '_HScrollbar'
            HorizontalScrollBar = GetElement(uiName, HorizontalScrollBarName)
            VerticleScrollBarName = elementName + '_VScrollbar'
            VerticleScrollBar = GetElement(uiName, VerticleScrollBarName)
            if HorizontalScrollBar and VerticleScrollBar:
                if elementName.find("PyMe_ListView_") == 0:
                    ScrollTarget.pack(side=tkinter.LEFT, fill=tkinter.BOTH, expand=True)
                    VerticleScrollBar.pack(side=tkinter.RIGHT, fill=tkinter.Y)
                    HorizontalScrollBar.pack(side=tkinter.TOP, fill=tkinter.BOTH, expand=True)
                else:
                    # ScrollTarget.place(x = 0,y = 0,width = event.width-20,height = event.height-20)
                    SetControlPlace(uiName, elementName, 0, 0, int(event.width * g_TKScaling) - 20,
                                    int(event.height * g_TKScaling) - 16)
                    VerticleScrollBar.place(x=event.width - 16, y=0, width=16, height=event.height - 16)
                    HorizontalScrollBar.place(x=0, y=event.height - 20, width=event.width, height=20)
            elif HorizontalScrollBar:
                if elementName.find("PyMe_ListView_") == 0:
                    ScrollTarget.pack(side=tkinter.TOP, fill=tkinter.BOTH, expand=True)
                    HorizontalScrollBar.pack(side=tkinter.BOTTOM, fill=tkinter.X)
                else:
                    # ScrollTarget.place(x = 0,y = 0,width = event.width,height = event.height-20)
                    SetControlPlace(uiName, elementName, 0, 0, int(event.width * g_TKScaling),
                                    int(event.height * g_TKScaling) - 20)
                    HorizontalScrollBar.place(x=0, y=event.height - 20, width=event.width, height=20)
            elif VerticleScrollBar:
                if elementName.find("PyMe_ListView_") == 0:
                    ScrollTarget.pack(side=tkinter.LEFT, fill=tkinter.BOTH, expand=True)
                    VerticleScrollBar.pack(side=tkinter.RIGHT, fill=tkinter.Y)
                else:
                    # ScrollTarget.place(x = 0,y = 0,width = event.width-20,height = event.height)
                    SetControlPlace(uiName, elementName, 0, 0, int(event.width * g_TKScaling) - 16,
                                    int(event.height * g_TKScaling))
                    VerticleScrollBar.place(x=event.width - 16, y=0, width=16, height=event.height)

        HorizontalScrollBarName = elementName + '_HScrollbar'
        HorizontalScrollBar = GetElement(uiName, HorizontalScrollBarName)
        VerticleScrollBarName = elementName + '_VScrollbar'
        VerticleScrollBar = GetElement(uiName, VerticleScrollBarName)
        if HorizontalScrollBar is None or VerticleScrollBar is None:
            ParentCanvas.bind('<Configure>', ResetVerticleScrollBar, add=True)
        ParentCanvas.event_generate("<Configure>")
...
def CreateUIFormJson(uiName, root, isTKroot, style, JsonString, SupportModule=True):
    ...
    if JsonString != '':
        try:
            ...
            JsonString = JsonString.replace('\x1b', '')
            ...
            if isTKroot == True:
                ...
                #     is_resizable_x, is_resizable_y = root.wm_resizable()
                #     if is_resizable_x == True or is_resizable_y == True:
                ...
                if 'ResolutionScaling' in UIInfoDict.keys() and UIInfoDict['ResolutionScaling'] == False:
                    default_font = tkinter.font.nametofont("TkDefaultFont")
                    default_font_size = round(14 / g_TKScaling + 0.51)
                ...
                if 'TransparentColor' in UIInfoDict.keys() and UIInfoDict['TransparentColor']:
                    ...
                    if 'RootTransparency' in UIInfoDict.keys():
                        ...
                        SetTransparencyFunction(root, 255)
                ...
                # ===WIN32-1512===
                if 'CustomTitleBarFile' in UIInfoDict.keys():
                    TitleBarFile = UIInfoDict['CustomTitleBarFile']
                    SetToolBar(root, TitleBarFile)
                # ================
            ...
            def SetElementLayout(uiName, ElementName, ElementInfo):
                if 'PackInfo' in ElementInfo:
                    SetControlPack(uiName, ElementName, ElementInfo['PackInfo'][0], ElementInfo['PackInfo'][1],
                                   ElementInfo['PackInfo'][2], ElementInfo['PackInfo'][3], ElementInfo['PackInfo'][4],
                                   ElementInfo['PackInfo'][5], ElementInfo['PackInfo'][6])
                elif 'GridInfo' in ElementInfo:
                    SetControlGrid(uiName, ElementName, ElementInfo['GridInfo'][0], ElementInfo['GridInfo'][1],
                                   ElementInfo['GridInfo'][2], ElementInfo['GridInfo'][3])
                else:
                    InFrameCanvas = False
                        ...
                        if ElementInfo['ParentName'] != 'Form_1':
                            ...
                            if ElementInfo['ParentName'].find("Frame") >= 0 and ElementName.find("Form_") < 0:
                                ...
                                if FrameCanvas and Control:
                                    if type(ElementInfo['PlaceInfo'][0]) == type(1) and type(
                                            ElementInfo['PlaceInfo'][1]) == type(1):
                                        if type(ElementInfo['PlaceInfo'][0]) == type(1) and type(
                                                ElementInfo['PlaceInfo'][3]) == type(1):
                                            if hasattr(Control, "GetWidget") == True:
                                                Control = Control.GetWidget()

                                            ControlHandle = FrameCanvas.create_window(ElementInfo['PlaceInfo'][0],
                                                                                      ElementInfo['PlaceInfo'][1],
                                                                                      window=Control, anchor=tkinter.NW)
                                            FrameCanvas.itemconfig(ControlHandle, width=ElementInfo['PlaceInfo'][2],
                                                                   height=ElementInfo['PlaceInfo'][3])
                                            InFrameCanvas = True
                                            PlaceDictionary = {}
                                            ...
                                            scrollregion = FrameCanvas.cget("scrollregion")
                                            if scrollregion == "":
                                                ParentFrameInfo = FrameCanvas.winfo_parent()
                                                ParentFrame = FrameCanvas._nametowidget(ParentFrameInfo)
                                                sw = ParentFrame.winfo_width()
                                                sh = ParentFrame.winfo_height()
                                                scrollregion = (0, 0, sw, sh)
                                            if type(scrollregion) == type("abc"):
                                                scrollregion = [int(num) for num in scrollregion.split()]
                                            if scrollregion:
                                                if scrollregion[0] > PlaceDictionary["x"]:
                                                    scrollregion = (
                                                    PlaceDictionary["x"], scrollregion[1], scrollregion[2],
                                                    scrollregion[3])
                                                if scrollregion[1] > PlaceDictionary["y"]:
                                                    scrollregion = (
                                                    scrollregion[0], PlaceDictionary["y"], scrollregion[2],
                                                    scrollregion[3])
                                                if scrollregion[2] < PlaceDictionary["x"] + PlaceDictionary["relwidth"]:
                                                    scrollregion = (scrollregion[0], scrollregion[1],
                                                                    PlaceDictionary["x"] + PlaceDictionary["relwidth"],
                                                                    scrollregion[3])
                                                if scrollregion[3] < PlaceDictionary["y"] + PlaceDictionary[
                                                    "relheight"]:
                                                    scrollregion = (scrollregion[0], scrollregion[1], scrollregion[2],
                                                                    PlaceDictionary["y"] + PlaceDictionary["relheight"])
                                                FrameCanvas.configure(scrollregion=scrollregion)

                    if InFrameCanvas == False:
                        SetControlPlace(uiName, ElementName, ElementInfo['PlaceInfo'][0], ElementInfo['PlaceInfo'][1],
                                        ElementInfo['PlaceInfo'][2], ElementInfo['PlaceInfo'][3],
                                        ElementInfo['PlaceInfo'][4], ElementInfo['PlaceInfo'][5],
                                        True)  # ElementInfo['PlaceInfo'][6])
            ...
            def SetElementFont(ElementInst, ElementInfo, IsEXUIControl=False):
                if 'Font' in ElementInfo.keys():
                    ...
                    newFont = tkinter.font.Font(family=FontInfo[0], size=int(FontInfo[1] / g_TKScaling),
                                                weight=FontInfo[2], slant=FontInfo[3], underline=FontInfo[4],
                                                overstrike=FontInfo[5])
                    ...
                    for TagInfo in ElementInfo['TagList']:
                        if ElementType == "TreeView" or ElementType == "ListView":
                            if len(TagInfo) == 2:
                                ...
                                ElementInst.tag_configure(TagInfo[0],
                                                          font=tkinter.font.Font(family=TagInfo[1], size=TagInfo[2],
                                                                                 weight=TagInfo[3], slant=TagInfo[4],
                                                                                 underline=TagInfo[5],
                                                                                 overstrike=TagInfo[6]))
                        else:
                            if len(TagInfo) == 2:
                                ElementInst.tag_config(TagInfo[0], foreground=TagInfo[1])
                            else:
                                ElementInst.tag_config(TagInfo[0],
                                                       font=tkinter.font.Font(family=TagInfo[1], size=TagInfo[2],
                                                                              weight=TagInfo[3], slant=TagInfo[4],
                                                                              underline=TagInfo[5],
                                                                              overstrike=TagInfo[6]),
                                                       foreground=TagInfo[7])
            ...
            def BuildTreeView(ElementInst, ParentItem, ParentList, IconDict):
                for ItemInfo in ParentList:
                    ...
                    if IconName in IconDict.keys():
                        if TagName:
                            ItemInfo[4] = ElementInst.insert(ParentItem, 'end', ItemName, text=ItemName,
                                                             image=IconDict[IconName], values=ItemValue, tag=TagName)
                        else:
                            ItemInfo[4] = ElementInst.insert(ParentItem, 'end', ItemName, text=ItemName,
                                                             image=IconDict[IconName], values=ItemValue)
                    else:
                        if TagName:
                            ItemInfo[4] = ElementInst.insert(ParentItem, 'end', ItemName, text=ItemName,
                                                             values=ItemValue, tag=TagName)
                        else:
                            ItemInfo[4] = ElementInst.insert(ParentItem, 'end', ItemName, text=ItemName,
                                                             values=ItemValue)
                ...
                for ElementInfo in ElementList:
                    ...
                    if ElementType == 'Form':
                        ...
                        if isTKroot == True:
                            ...
                            Form_1 = tkinter.Canvas(root, heigh=ElementInfo['Size'][1])
                            Form_1.pack(side=tkinter.TOP, fill=tkinter.BOTH)
                        # ================
                        Register(uiName, 'Form_1', Form_1)
                        Form_1_BGColor = '#f8f9fa'
                        ...
                        if 'ScrollBarList' in ElementInfo:
                            CanvasParent = tkinter.Canvas(ParentElement)
                            CanvasParent_BGColor = ParentElement.cget('bg')
                            CanvasParent.configure(bg=CanvasParent_BGColor)
                            CanvasParentName = ElementName + "_ParentCanvas"
                            Register(uiName, CanvasParentName, CanvasParent, ElementAliasName + "_ParentCanvas",
                                     GroupName, StyleName)
                            # ListViewParent.pack(side=tkinter.TOP,fill='both',expand=True)
                            SetElementLayout(uiName, CanvasParentName, ElementInfo)
                        else:
                            CanvasParent = ParentElement
                        NewElementInst = tkinter.Canvas(CanvasParent)
                        Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                        if CanvasParent is ParentElement:
                        ...
                        if ElementType == 'LabelFrame':
                            if 'Style' in ElementInfo.keys():
                                NewElementInst = tkinter.ttk.LabelFrame(CanvasParent, text=ElementInfo['Text'],
                                                                        style=ElementInfo['Style'])
                            else:
                                if 'PackInfo' in ElementInfo.keys():
                                    NewElementInst = tkinter.LabelFrame(CanvasParent, text=ElementInfo['Text'],
                                                                        width=ElementInfo['Size'][0],
                                                                        height=ElementInfo['Size'][1], takefocus=True)
                                else:
                                    NewElementInst = tkinter.LabelFrame(CanvasParent, text=ElementInfo['Text'],
                                                                        takefocus=True)
                        else:
                            if 'Style' in ElementInfo.keys():
                                NewElementInst = tkinter.ttk.Frame(CanvasParent, style=ElementInfo['Style'])
                            else:
                                if 'PackInfo' in ElementInfo.keys():
                                    NewElementInst = tkinter.Frame(CanvasParent, width=ElementInfo['Size'][0],
                                                                   height=ElementInfo['Size'][1])
                                else:
                                    NewElementInst = tkinter.Frame(CanvasParent)
                            ...
                            if FrameCanvas:
                                if EmbedUI:
                                    ...
                                    for widgetName in ChildWidgetList.keys():
                                        ...
                                        ChildHandle = FrameCanvas.create_window(0, 0, window=uiDialogForm1,
                                                                                anchor=tkinter.NW, tag="Form_1")
                                        FrameCanvas.itemconfig(ChildHandle, width=uiDialogWidth, height=uiDialogHeight)
                                        if hasattr(EmbedUI, "Configure") == True:
                                            EmbedUI.Configure(ChartEvent(uiDialogWidth, uiDialogHeight, uiDialogForm1))
                        ...
                        if 'AutoWrap' in ElementInfo.keys():
                            if 'Style' in ElementInfo.keys():
                                if 'PlaceInfo' in ElementInfo.keys() and type(ElementInfo['PlaceInfo'][2]) == type(1):
                                    NewElementInst = tkinter.ttk.Message(ParentElement, text=ElementInfo['Text'],
                                                                         style=ElementInfo['Style'],
                                                                         width=ElementInfo['PlaceInfo'][2])
                                else:
                                    NewElementInst = tkinter.ttk.Message(ParentElement, text=ElementInfo['Text'],
                                                                         style=ElementInfo['Style'])
                            else:
                            ...
                            if 'Style' in ElementInfo.keys():
                                NewElementInst = tkinter.ttk.Label(ParentElement, text=ElementInfo['Text'],
                                                                   style=ElementInfo['Style'])
                            else:
                        ...
                        if 'ScrollBarList' in ElementInfo:
                            ListBoxParent = tkinter.Canvas(ParentElement)
                            ListBoxParent_BGColor = ParentElement.cget('bg')
                            ListBoxParent.configure(bg=ListBoxParent_BGColor)
                            ListBoxParentName = ElementName + "_ParentCanvas"
                            Register(uiName, ListBoxParentName, ListBoxParent, ElementAliasName + "_ParentCanvas",
                                     GroupName, StyleName)
                            # ListBoxParent.pack(side=tkinter.TOP,fill='both',expand=True)
                            SetElementLayout(uiName, ListBoxParentName, ElementInfo)
                        else:
                            ListBoxParent = ParentElement
                        NewElementInst = tkinter.Listbox(ListBoxParent)
                        Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                        if ListBoxParent is ParentElement:
                            SetElementLayout(uiName, ElementName, ElementInfo)
                        ...
                        if 'SelectMode' in ElementInfo.keys():
                            ...
                            elif ElementInfo['SelectMode'] == 'MULTIPLE':
                                NewElementInst.configure(selectmode=tkinter.MULTIPLE)
                        ...
                        if 'Style' in ElementInfo.keys():
                            NewElementInst = tkinter.ttk.Combobox(ParentElement, textvariable=ComboBoxVariable,
                                                                  state='normal', style=ElementInfo['Style'])
                            ...
                            NewElementInst = tkinter.ttk.Combobox(ParentElement, textvariable=ComboBoxVariable,
                                                                  state='normal', style=ElementAliasName + '.TCombobox')
                            if StyleName is None:
                        ...
                        if GroupName:
                            ...
                            NewElementInst = tkinter.Radiobutton(ParentElement, variable=Group_Variable,
                                                                 value=ElementInfo['Value'], text=ElementInfo['Text'],
                                                                 anchor=tkinter.W)
                        else:
                            NewElementInst = tkinter.Radiobutton(ParentElement, value=ElementInfo['Value'],
                                                                 text=ElementInfo['Text'], anchor=tkinter.W)
                        Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                        SetElementLayout(uiName, ElementName, ElementInfo)
                        if 'BGColor' in ElementInfo.keys():
                            NewElementInst.configure(bg=ElementInfo['BGColor'])
                        ...
                        if 'SelfDrawing' in ElementInfo.keys():
                            SetRadioButtonPyMeStyle(uiName, ElementName, ElementInfo['Value'], FGColor,
                                                    ElementInfo['SelectFGColor'], ElementInfo['Visible'])
                        else:
                            NewElementInst.bind("<ButtonRelease-1>", lambda event: OnRadioButtonSelectedFGColor(event),
                                                add=True)
                        ...
                        NewElementInst = tkinter.Checkbutton(ParentElement, variable=CheckButtonVariable,
                                                             text=ElementInfo['Text'], anchor=tkinter.W)
                        Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                        SetElementLayout(uiName, ElementName, ElementInfo)
                        if 'BGColor' in ElementInfo.keys():
                        ...
                        if 'SelectImage' in ElementInfo.keys():
                            SetImage(uiName, ElementName, ElementInfo['SelectImage'], True, 'RGBA', 'selected')
                        if 'SelfDrawing' in ElementInfo.keys():
                            SetCheckButtonPyMeStyle(uiName, ElementName, ElementInfo['Value'], FGColor,
                                                    ElementInfo['SelectFGColor'], ElementInfo['Visible'])
                        if 'Compound' in ElementInfo.keys():
                            NewElementInst.configure(compound=ElementInfo['Compound'])
                        if 'Relief' in ElementInfo.keys():
                        ...
                        NewElementInst = tkinter.Scale(ParentElement, orient=ElementInfo['Orient'],
                                                       from_=ElementInfo['From'], to=ElementInfo['To'],
                                                       bigincrement=ElementInfo["BigIncrement"],
                                                       length=ElementInfo["Length"])
                        Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                        SetElementLayout(uiName, ElementName, ElementInfo)
                        if 'BGColor' in ElementInfo.keys():
                            NewElementInst.configure(bg=ElementInfo['BGColor'])
                        ...
                        if 'ValueList' in ElementInfo.keys():
                            ...
                            AddUserData(uiName, ElementName, ElementName + "_ValueList", "list",
                                        ElementInfo['ValueList'])
                        if 'Relief' in ElementInfo.keys():
                            NewElementInst.configure(relief=ElementInfo['Relief'])
                        if 'State' in ElementInfo.keys():
                            NewElementInst.configure(state=ElementInfo['State'])
                        ...
                        Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                        if 'MinValue' in ElementInfo.keys():
                            NewElementInst.SetMinValue(ElementInfo['MinValue'], False)
                        ...
                        if 'Font' in ElementInfo.keys():
                            FontInfo = ElementInfo['Font']
                            newFont = tkinter.font.Font(family=FontInfo[0], size=FontInfo[1], weight=FontInfo[2],
                                                        slant=FontInfo[3], underline=FontInfo[4],
                                                        overstrike=FontInfo[5])
                            NewElementInst.SetFont(newFont, False)
                        NewElementInst.Rebuild()
                    elif ElementType == 'TreeView':
                        if 'ScrollBarList' in ElementInfo:
                            ListViewParent = tkinter.Canvas(ParentElement)
                            ListViewParent_BGColor = ParentElement.cget('bg')
                            ListViewParent.configure(bg=ListViewParent_BGColor)
                            ListViewParentName = ElementName + "_ParentCanvas"
                            Register(uiName, ListViewParentName, ListViewParent, ElementAliasName + "_ParentCanvas",
                                     GroupName, StyleName)
                            # ListViewParent.pack(side=tkinter.TOP,fill='both',expand=True)
                            SetElementLayout(uiName, ListViewParentName, ElementInfo)
                            # CanvasWidth = ListViewParent.winfo_width()
                            # CanvasHeight = ListViewParent.winfo_height()
                        else:
                            ListViewParent = ParentElement
                        NewElementInst = tkinter.ttk.Treeview(ListViewParent, show="tree")
                        Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                        if ListViewParent is ParentElement:
                            SetElementLayout(uiName, ElementName, ElementInfo)
                        if 'SelectMode' in ElementInfo.keys():
                            if ElementInfo['SelectMode'] == 'NONE':
                                NewElementInst.configure(selectmode=tkinter.NONE)
                            elif ElementInfo['SelectMode'] == 'BROWSE':
                                NewElementInst.configure(selectmode=tkinter.BROWSE)
                            else:
                                NewElementInst.configure(selectmode=tkinter.EXTENDED)
                        ...
                        ListViewParent_TOP = None
                        ListViewParent_BOTTOM = None
                        if 'ScrollBarList' in ElementInfo:
                            ListViewParent = tkinter.Canvas(ParentElement)
                            ListViewParent_BGColor = ParentElement.cget('bg')
                            ListViewParent.configure(bg='#000000')
                            ListViewParentName = ElementName + "_ParentCanvas"
                            Register(uiName, ListViewParentName, ListViewParent, ElementAliasName + "_ParentCanvas",
                                     GroupName, StyleName)
                            # ListViewParent.pack(side=tkinter.TOP,fill='both',expand=True)
                            SetElementLayout(uiName, ListViewParentName, ElementInfo)
                            if ElementInfo['ScrollBarList'][0] == True and ElementInfo['ScrollBarList'][1] == True:
                                ListViewParent_TOP = tkinter.Canvas(ListViewParent, bg='#FF0000')
                                ListViewParent_TOP.pack(side=tkinter.TOP, fill=tkinter.BOTH, expand=True)
                                ListViewParent_BOTTOM = tkinter.Canvas(ListViewParent, height=20, bg='#00FF00')
                                ListViewParent_BOTTOM.pack(side=tkinter.BOTTOM, fill=tkinter.X)
                        else:
                            ListViewParent = ParentElement
                        ...
                        if 'RowHeight' in ElementInfo.keys():
                            style.configure(StyleName, rowheight=int(ElementInfo['RowHeight']))

                        if ListViewParent_TOP and ListViewParent_BOTTOM:
                            NewElementInst = tkinter.ttk.Treeview(ListViewParent_TOP, show="headings", style=StyleName,
                                                                  height=0)
                            Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                            if ListViewParent is ParentElement:
                                SetElementLayout(uiName, ElementName, ElementInfo)
                            else:
                                NewElementInst.pack(side=tkinter.LEFT, fill=tkinter.BOTH, expand=True)
                        else:
                            NewElementInst = tkinter.ttk.Treeview(ListViewParent, show="headings", style=StyleName,
                                                                  height=0)
                            Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                            if ListViewParent is ParentElement:
                                SetElementLayout(uiName, ElementName, ElementInfo)
                                ...
                                if ElementInfo['ScrollBarList'][0] == True:
                                    NewElementInst.pack(side=tkinter.TOP, fill=tkinter.BOTH, expand=True)
                                if ElementInfo['ScrollBarList'][1] == True:
                                    NewElementInst.pack(side=tkinter.LEFT, fill=tkinter.BOTH, expand=True)
                        ...
                        if 'Font' in ElementInfo.keys():
                            ...
                            newFont = tkinter.font.Font(family=FontInfo[0], size=FontInfo[1], weight=FontInfo[2],
                                                        slant=FontInfo[3], underline=FontInfo[4],
                                                        overstrike=FontInfo[5])
                            HeadingStyleName = StyleName + ".Heading"
                            style.configure(HeadingStyleName, font=newFont)
                            NewElementInst.configure(style=StyleName)
                        ...
                        if 'ScrollBarList' in ElementInfo:
                            if ElementInfo['ScrollBarList'][0] == True:
                                SetElementScrollbar(uiName, ElementName, tkinter.HORIZONTAL, ListViewParent_TOP,
                                                    ListViewParent_BOTTOM)
                            if ElementInfo['ScrollBarList'][1] == True:
                                SetElementScrollbar(uiName, ElementName, tkinter.VERTICAL, ListViewParent_TOP,
                                                    ListViewParent_BOTTOM)
                        ...
                        style.map(StyleName + ".Tab", background=[("selected", SelectedBGColor)],
                                  foreground=[("selected", SelectedFGColor)])
                        NewElementInst.configure(style=StyleName)
                        ...
                        Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                        SetElementLayout(uiName, ElementName, ElementInfo)
                    elif ElementType == 'Win32':
                        IsPyMeModule = True
                        if SupportModule == True:
                            NewElementInst = Win32()
                            Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                        ...
                        if SupportModule == True:
                            ...
                            if DataType == 'TXT':
                                NewElementInst = FileReader_ReadTXTFile(filename=filename, encoding=encoding,
                                                                        uiName=uiName, elementName=TargetElementName)
                            elif DataType == 'CSV':
                                NewElementInst = FileReader_ReadCSVFile(filename=filename, encoding=encoding,
                                                                        uiName=uiName, elementName=TargetElementName)
                            elif DataType == 'XML':
                                NewElementInst = FileReader_ReadXMLFile(filename=filename, encoding=encoding,
                                                                        uiName=uiName, elementName=TargetElementName)
                            elif DataType == 'JSON':
                                NewElementInst = FileReader_ReadJSONFile(filename=filename, encoding=encoding,
                                                                         uiName=uiName, elementName=TargetElementName)
                            elif DataType == 'WEB':
                                NewElementInst = FileReader_ReadWEBFile(fileorurl=fileorurl, encoding=encoding,
                                                                        uiName=uiName, elementName=TargetElementName)
                            else:
                        ...
                        if SupportModule == True:
                            ...
                            # ===WIN32-1512===
                            while findNewPort:
                                for uiName_loop in G_UIElementDictionary:
                                    for elementName in G_UIElementDictionary[uiName_loop]:
                                        splitArray = elementName.split('_')
                                        if len(splitArray) > 1:
                                            if splitArray[0] == "Socket" or splitArray[1] == "Socket":
                                                SocketInstance = G_UIElementDictionary[uiName_loop][elementName]
                            ...
                            NewElementInst = Socket(protocol=socket_protocol, ip=socket_ip, port=socket_port,
                                                    listen=int(socket_listen), encoding=socket_encoding,
                                                    buffsize=int(socket_buffsize), netMsgTable=socket_netMsgTable)
                            # 名称有问题
                        ...
                        if SupportModule == True:
                            ...
                            NewElementInst = MQTT(broker=mqtt_broker, port=int(mqtt_port), username=mqtt_username,
                                                  password=mqtt_password, keeplive=mqtt_keeplive, block=mqtt_block,
                        ...
                        if SupportModule == True:
                            ...
                            NewElementInst = SSH(hostname=ssh_hostname, username=ssh_username, password=ssh_password,
                        ...
                        if SupportModule == True:
                            ...
                            NewElementInst = Serial(serialport=serial_port, baudrate=int(serial_baudrate),
                                                    databit=int(serial_databit), parity=serial_parity,
                                                    stopbit=serial_stopbit, readbufsize=int(serial_readbufsize),
                                                    sendbufsize=int(serial_sendbufsize), encoding=serial_encoding)
                            # 名称有问题
                        ...
                        if SupportModule == True:
                            ...
                            NewElementInst = USB(readbufsize=int(usb_readbufsize), sendbufsize=int(usb_sendbufsize),
                                                 encoding=usb_encoding)
                            # 名称有问题
                        ...
                        if SupportModule == True:
                            ...
                            NewElementInst = SMTP(server=smtp_server, port=int(smtp_port), email=smtp_email,
                        ...
                        if SupportModule == True:
                            ...
                            NewElementInst = VideoPlayer(uiName=uiName, elementName=canvasname, filename=filename,
                                                         vlcpath=vlcpath)
                            Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                            ...
                            if 'datatype' in ElementInfo.keys():
                                ...
                                if NewElementInst.BindingListView(uiName, TargetElementName) == True:
                                    if DataType == 'EXCEL':
                                        ...
                                        NewElementInst.OpenMYSQL(ElementInfo['ip'], ElementInfo['port'],
                                                                 ElementInfo['user'], ElementInfo['password'],
                                                                 ElementInfo['database'], ElementInfo['sheetname'])
                                    elif DataType == 'SQLSERVER':
                                        NewElementInst.OpenSQLSERVER(ElementInfo['ip'], ElementInfo['port'],
                                                                     ElementInfo['user'], ElementInfo['password'],
                                                                     ElementInfo['database'], ElementInfo['sheetname'])
                            Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                        ...
                        if SupportModule == True:
                            ...
                            NewElementInst = Selenium(drivertype=drivertype, url=url, headless=headless,
                                                      profile_dir=profile_dir, proxy_path=proxy_path,
                        ...
                        if SupportModule == True:
                            ...
                            Register(uiName, ElementName, NewElementInst, ElementAliasName, GroupName, StyleName)
                    elif ElementType == 'AIChat':
                        IsPyMeModule = True
                        if SupportModule == True:
                            NewElementInst = AIChat()
                            TargetElementName = None
                            if 'elementName' in ElementInfo.keys():
                                TargetElementName = ElementInfo['elementName']
                                if TargetElementName:
                ...
                for ElementInfo in ElementList:
                    ...
                    if ElementType == 'RadioButton':
                        if 'SelectBGColor' in ElementInfo.keys() and 'SelectFGColor' in ElementInfo.keys():
                            SetRadioButtonSelectedColor(uiName, ElementName, ElementInfo['SelectBGColor'],
                                                        ElementInfo['SelectFGColor'])
            # 最后再遍历所有的控件事件进行处理,主要是个别函数如LoadData这时可能引发某些逻辑找不到控件的情况，所以放在最后处理
            if 'WidgetList' in UIInfoDict.keys():
                ElementList = UIInfoDict['WidgetList']
                for ElementInfo in ElementList:
                    ...
                    if ElementType == 'RadioButton':
                        ...
                        if 'SelectBGColor' in ElementInfo.keys() and 'SelectFGColor' in ElementInfo.keys():
                            SetCheckButtonSelectedColor(uiName, ElementName, ElementInfo['SelectBGColor'],
                                                        ElementInfo['SelectFGColor'])
                    elif ElementType == 'LabelFrame' or ElementType == 'Frame':
                                    ...
                                    if FrameCanvas:
                                        ...
                                        for widgetTag in widgetNameTagList:
                                            ...
                                            if (y2 > uiDialogHeight):
                                                uiDialogHeight = y2
                                        FrameCanvas.config(scrollregion=(
                                        0, 0, uiDialogWidth + HScrollWidth, uiDialogHeight + VScrollHeight))
                    # 加载函数
                    if 'EventList' in ElementInfo.keys():
                            ...
                            if hasattr(UICmd, funcName) == True:
                                ...
                                if ElementType == 'Form':
                                    if 'Load' == eventName or 'InitCheck' == eventName or 'Display' == eventName or 'Exit' == eventName or 'Configure' == eventName:
                                        continue
                                    if 'Drop' == eventName:
                                        SetControlAcceptDrop(uiName=uiName, elementName=ElementInfo['AliasName'],
                                                             functionCallback=eventFunc)
                                        continue
                                    if eventName in G_TKKey:
                                        root.bind('<' + eventName + '>',
                                                  EventFunction_Adaptor(eventFunc, uiName=uiName, widgetName="Form_1"))
                                    else:
                                        Form_1.bind('<' + eventName + '>',
                                                    EventFunction_Adaptor(eventFunc, uiName=uiName,
                                                                          widgetName="Form_1"))
                                    continue
                                if 'ListboxSelect' == eventName:
                                    NewElementInst.bind('<<' + eventName + '>>',
                                                        EventTwoFunction_Adaptor(OnListBoxSelect, eventFunc,
                                                                                 uiName=uiName,
                                                                                 widgetName=ElementInfo['AliasName']))
                                    ...
                                    if ElementType == 'ListBox':
                                        EventTwoFunction_Adaptor(OnListBoxFocusOut, eventFunc, uiName=uiName,
                                                                 widgetName=ElementInfo['AliasName'])
                                    else:
                                        NewElementInst.bind('<' + eventName + '>',
                                                            EventFunction_Adaptor(eventFunc, uiName=uiName,
                                    ...
                                    if ElementType == 'ListView':
                                        EventTwoFunction_Adaptor(OnListViewRowMouseMotion, eventFunc, uiName=uiName,
                                                                 widgetName=ElementInfo['AliasName'])
                                    else:
                                        NewElementInst.bind('<' + eventName + '>',
                                                            EventFunction_Adaptor(eventFunc, uiName=uiName,
                                    ...
                                    NewElementInst.SetBtnCallBackFunction(eventFunc, uiName=uiName,
                                                                          widgetName=ElementInfo['AliasName'])
                                elif 'Switch' == eventName:
                                    NewElementInst.SetSwitchCallBackFunction(eventFunc, uiName=uiName,
                                                                             widgetName=ElementInfo['AliasName'])
                                elif 'ItemSelect' == eventName:
                                    if ElementType == 'LabelButton':
                                        NewElementInst.SetDropListCallBackFunction(eventFunc, uiName=uiName,
                                                                                   widgetName=ElementInfo['AliasName'])
                                    elif ElementType == 'Navigation':
                                        NewElementInst.SetNavigationCallBackFunction(eventFunc, uiName=uiName,
                                                                                     widgetName=ElementInfo[
                                                                                         'AliasName'])
                                    elif ElementType == 'ShowCase':
                                        NewElementInst.SetClickItemCallBackFunction(eventFunc, uiName=uiName,
                                                                                    widgetName=ElementInfo['AliasName'])
                                    elif ElementType == 'ListMenu':
                                        NewElementInst.SetListMenuCallBackFunction(eventFunc, uiName=uiName,
                                                                                   widgetName=ElementInfo['AliasName'])
                                    else:
                                        NewElementInst.bind('<' + eventName + '>',
                                                            EventFunction_Adaptor(eventFunc, uiName=uiName,
                                    ...
                                    NewElementInst.SetPageClickCallBackFunction(eventFunc, uiName=uiName,
                                                                                widgetName=ElementInfo['AliasName'])
                                elif 'CellClicked' == eventName:
                                    NewElementInst.bind("<Button-1>",
                                                        EventFunction_Adaptor(OnListViewCellClicked, uiName=uiName,
                                                                              widgetName=ElementInfo['AliasName'],
                                                                              callbackFunc=eventFunc), add=True)
                                elif 'CellDoubleClicked' == eventName:
                                    NewElementInst.bind("<Double-1>", EventFunction_Adaptor(OnListViewCellDoubleClicked,
                                                                                            uiName=uiName,
                                                                                            widgetName=ElementInfo[
                                                                                                'AliasName'],
                                    ...
                                    if 'ColumnList' in ElementInfo.keys():
                                        ...
                                        for ColumnInfo in ColumnList:
                                            ...
                                            ColumnWidth = ColumnInfo[2]
                                            ColumnStretch = ColumnInfo[3]
                                            NewElementInst.heading(ColumnName,
                                                                   command=partial(ListViewHeadingFunction_Adaptor,
                                                                                   fun=eventFunc, uiName=uiName,
                                                                                   widgetName=ElementName,
                                                                                   columnname=ColumnName))
                                elif 'Timer' == eventName:
                                    NewElementInst.SetTimerCallBackFunction(eventFunc, uiName=uiName,
                                                                            widgetName=ElementInfo['AliasName'])
                                    ...
                                    SetClickXYFunction(uiName=uiName, elementName=ElementInfo['AliasName'],
                                                       callBackFunction=eventFunc)
                                elif 'Drop' == eventName:
                                    SetControlAcceptDrop(uiName=uiName, elementName=ElementInfo['AliasName'],
                                                         functionCallback=eventFunc)
                                elif 'LeftIconClicked' == eventName:
                                    NewElementInst.SetLeftIconClickFunction(eventFunc, uiName=uiName,
                                                                            widgetName=ElementInfo['AliasName'])
                                elif 'RightIconClicked' == eventName:
                                    NewElementInst.SetRightIconClickFunction(eventFunc, uiName=uiName,
                                                                             widgetName=ElementInfo['AliasName'])
                                elif 'TextChanged' == eventName:
                                    NewElementInst.SetTextChangedFunction(eventFunc, uiName=uiName,
                                                                          widgetName=ElementInfo['AliasName'])
                                elif 'ValueChanged' == eventName:
                                    NewElementInst.SetValueChangedFunction(eventFunc, uiName=uiName,
                                                                           widgetName=ElementInfo['AliasName'])
                                elif 'Command' == eventName:
                                    if ElementType == 'Scale':
                                        NewElementInst.configure(
                                            command=SetValueChangedFunction(eventFunc, uiName=uiName,
                                                                            widgetName=ElementInfo['AliasName']))
                                    elif ElementType == 'SpinBox' or ElementType == 'Button':
                                        NewElementInst.configure(
                                            command=partial(CommandFunction_Adaptor, fun=eventFunc, uiName=uiName,
                                                            widgetName=ElementInfo['AliasName']))
                                    elif ElementType == 'LabelButton':
                                        NewElementInst.SetCommandFunction(eventFunc, uiName=uiName,
                                                                          widgetName=ElementInfo['AliasName'])
                                    else:
                                        NewElementInst.configure(command=partial(eventFunc, uiName=uiName,
                                                                                 widgetName=ElementInfo['AliasName']))
                ...
                for MenuInfo in MenuInfoList:
                    ...
                    if MenuType == 'cascade':
                        ...
                        if imageFile and MenuFont:
                            ...
                            ParentMenu.add_cascade(label=title, font=MenuFont, image=MenuIconDict[IconName],
                                                   compound='left', menu=SubMenu)
                        elif imageFile:
                            IconName = 'MainMenu_' + shotname
                            ParentMenu.add_cascade(label=title, image=MenuIconDict[IconName], compound='left',
                                                   menu=SubMenu)
                        elif MenuFont:
                        ...
                        if eventFunc:
                            if imageFile and MenuFont and AccKey:
                                IconName = 'MainMenu_' + shotname
                                ParentMenu.add_checkbutton(label=title, font=MenuFont, image=MenuIconDict[IconName],
                                                           compound='left', accelerator=AccKey,
                                                           variable=CheckButton_Var,
                                                           command=lambda: onCheckButton(eventFunc, uiName=uiName,
                                                                                         itemName=title,
                                                                                         CheckButton_Var=CheckButton_Var))
                            elif imageFile and MenuFont:
                                IconName = 'MainMenu_' + shotname
                                ParentMenu.add_checkbutton(label=title, font=MenuFont, image=MenuIconDict[IconName],
                                                           compound='left', variable=CheckButton_Var,
                                                           command=lambda: onCheckButton(eventFunc, uiName=uiName,
                                                                                         itemName=title,
                                                                                         CheckButton_Var=CheckButton_Var))
                            elif MenuFont and AccKey:
                                IconName = 'MainMenu_' + shotname
                                ParentMenu.add_checkbutton(label=title, font=MenuFont, accelerator=AccKey,
                                                           variable=CheckButton_Var,
                                                           command=lambda: onCheckButton(eventFunc, uiName=uiName,
                                                                                         itemName=title,
                                                                                         CheckButton_Var=CheckButton_Var))
                                ...
                                IconName = 'MainMenu_' + shotname
                                ParentMenu.add_checkbutton(label=title, image=MenuIconDict[IconName], compound='left',
                                                           accelerator=AccKey, variable=CheckButton_Var,
                                                           command=lambda: onCheckButton(eventFunc, uiName=uiName,
                                                                                         itemName=title,
                                                                                         CheckButton_Var=CheckButton_Var))
                            elif imageFile:
                                ...
                                ParentMenu.add_checkbutton(label=title, font=MenuFont, variable=CheckButton_Var,
                                                           command=lambda: onCheckButton(eventFunc=eventFunc,
                                                                                         uiName=uiName, itemName=title,
                                                                                         CheckButton_Var=CheckButton_Var))
                            else:
                                ParentMenu.add_checkbutton(label=title, variable=CheckButton_Var,
                                                           command=lambda: onCheckButton(eventFunc=eventFunc,
                                                                                         uiName=uiName, itemName=title,
                                                                                         CheckButton_Var=CheckButton_Var))
                            if AccKey != '':
                                root.bind("<" + ConvAccEventKey(AccKey, 'B') + ">",
                                          MenuFunction_Adaptor(onCheckButton, uiName=uiName, itemName=title,
                                                               CheckButton_Var=CheckButton_Var))
                                root.bind("<" + ConvAccEventKey(AccKey, 'S') + ">",
                                          MenuFunction_Adaptor(onCheckButton, uiName=uiName, itemName=title,
                                                               CheckButton_Var=CheckButton_Var))
                    else:  # command
                        title = MenuInfo[1]
                        ...
                        if eventFunc:
                            if imageFile and MenuFont and AccKey:
                                IconName = 'MainMenu_' + shotname
                                ParentMenu.add_command(label=title, font=MenuFont, image=MenuIconDict[IconName],
                                                       compound='left', accelerator=AccKey,
                                                       command=partial(eventFunc, uiName=uiName, itemName=title))
                            elif imageFile and MenuFont:
                                IconName = 'MainMenu_' + shotname
                                ParentMenu.add_command(label=title, font=MenuFont, image=MenuIconDict[IconName],
                                                       compound='left',
                                ...
                                IconName = 'MainMenu_' + shotname
                                ParentMenu.add_command(label=title, font=MenuFont, accelerator=AccKey,
                                                       command=partial(eventFunc, uiName=uiName, itemName=title))
                            elif imageFile and AccKey:
                                IconName = 'MainMenu_' + shotname
                                ParentMenu.add_command(label=title, image=MenuIconDict[IconName], compound='left',
                                                       accelerator=AccKey,
                                                       command=partial(eventFunc, uiName=uiName, itemName=title))
                            elif imageFile:
                                IconName = 'MainMenu_' + shotname
                                ...
                                ParentMenu.add_command(label=title, font=MenuFont,
                                                       command=partial(eventFunc, uiName=uiName, itemName=title))
                            else:
                                ParentMenu.add_command(label=title,
                                                       command=partial(eventFunc, uiName=uiName, itemName=title))
                            if AccKey != '':
                                root.bind("<" + ConvAccEventKey(AccKey, 'B') + ">",
                                          MenuFunction_Adaptor(eventFunc, uiName=uiName, itemName=title))
                                root.bind("<" + ConvAccEventKey(AccKey, 'S') + ">",
                                          MenuFunction_Adaptor(eventFunc, uiName=uiName, itemName=title))
        ...
        if isTKroot == True:
            if UIInfoDict['WindowResizable'] == False:
                ...
                if is_resizable_x == True or is_resizable_y == True:
                    root.resizable(False, False)
            SetTitleBar(root, titleText=UIInfoDict['WindowTitle'], isDarkMode=UIInfoDict['DarkMode'],
                        isDropTitle=UIInfoDict['DropTitle'])
            CenterDlg(uiName, root, UIInfoDict['WindowSize'][0], UIInfoDict['WindowSize'][1], False,
                      UIInfoDict['WindowPosition'])
            WindowDraggable(root, UIInfoDict['DragWindow'], UIInfoDict['BorderWidth'], UIInfoDict['BorderColor'])
    if style:
...
def CleanUIDictionaryInfo(uiName):
...
def ReleaseEmbedUI(uiName):
    PrintFunctionInfo(ReleaseEmbedUI, [uiName])
    # 删除嵌入的界面
    if uiName in G_UIElementUserDataArray:
    ...
    if Visible == True:
        if G_UIElementPlaceDictionary[uiName][RealElementName]["type"] == "pack":
            ...
            if 'window_id' in G_UIElementPlaceDictionary[uiName][RealElementName]:
                ...
                HScrollName = RealElementName + "_HScrollbar"
                VScrollName = RealElementName + "_VScrollbar"
                HScrollbar = GetElement(uiName, HScrollName)
                VScrollbar = GetElement(uiName, VScrollName)
                if HScrollbar or VScrollbar:
                    ParentInfo = Control.winfo_parent()
                    ParentCanvas = Control._nametowidget(ParentInfo)
                    ParentCanvas.event_generate("<Configure>")
    else:
        if G_UIElementPlaceDictionary[uiName][RealElementName]["type"] == "pack":
            Control.pack_forget()
        ...
        HScrollbar = GetElement(uiName, HScrollName)
        if HScrollbar:
            HScrollbar.place_forget()
        VScrollbar = GetElement(uiName, VScrollName)
        if VScrollbar:
            VScrollbar.place_forget()
...
def SetElementEnable(uiName, elementName, Enable):
    ...
    if element:
        if elementName and uiName in G_UIElementAliasDictionary.keys() and elementName in G_UIElementAliasDictionary[
            uiName].keys():
            elementName = G_UIElementAliasDictionary[uiName][elementName]
...
def QueryText(uiName, elementName, textValue='', fromPosition='1.0', toPosition=tkinter.END):
    PrintFunctionInfo(QueryText, [uiName, elementName, textValue, fromPosition, toPosition])
    # 查询文字在Text中的位置
    element = GetElement(uiName, elementName)
    if element:
        if uiName in G_UIElementAliasDictionary.keys() and elementName in G_UIElementAliasDictionary[uiName].keys():
    ...
    if uiName in G_UIElementDictionary:
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('Text_') >= 0:
                if hasattr(element, "GetEntry") == True:
                    element = element.GetEntry()
                # 搜索从开始到结束的所有"hello"出现位置
                pos = element.search(textValue, fromPosition, stopindex=toPosition)
                return pos
    return None


def GetContent(uiName, elementName, fromPosition='1.0', toPosition=tkinter.END):
    PrintFunctionInfo(QueryText, [uiName, elementName, fromPosition, toPosition])
    # 获取Text中的文字内容
    element = GetElement(uiName, elementName)
    if element:
        if uiName in G_UIElementAliasDictionary.keys() and elementName in G_UIElementAliasDictionary[uiName].keys():
    ...
    if uiName in G_UIElementDictionary:
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('Text_') >= 0:
                if hasattr(element, "GetEntry") == True:
                    element = element.GetEntry()
                    return element.get(fromPosition, toPosition)
    return None
...
def CreateFont(fontName, fontSize, fontWeight=False, fontSlant=False, fontUnderline=False, fontOverstrike=False):
    ...
    if type(fontOverstrike) == type(bool):
        if fontOverstrike == True:
            ...
            fontOverstrike = 0
    return tkinter.font.Font(family=fontName, size=fontSize, weight=fontWeight, slant=fontSlant,
                             underline=fontUnderline, overstrike=fontOverstrike)


def SetFont(uiName, elementName, fontName, fontSize, fontWeight='normal', fontSlant='roman', fontUnderline=0,
            fontOverstrike=0):
    PrintFunctionInfo(SetFont,
                      [uiName, elementName, fontName, fontSize, fontWeight, fontSlant, fontUnderline, fontOverstrike])
    # 设置控件字体
    if type(fontWeight) == type(bool):
        if fontWeight == True:
            fontWeight = 'bold'
        ...
        if elementName in G_UIElementDictionary[uiName]:
            ...
            if elementName in G_CanvasFontDictionary[uiName]:
                for fontInfo in G_CanvasFontDictionary[uiName][elementName]:
                    if fontInfo[1] == fontName and fontInfo[2] == str(fontSize) and fontInfo[3] == fontWeight and \
                            fontInfo[4] == fontSlant and fontInfo[5] == str(fontUnderline) and fontInfo[6] == str(
                            fontOverstrike):
                        newFont = fontInfo[0]
                        break
            else:
            ...
            if newFont is None:
                newFont = tkinter.font.Font(family=fontName, size=fontSize, weight=fontWeight, slant=fontSlant,
                                            underline=fontUnderline, overstrike=fontOverstrike)
            if elementName.find('Entry_') >= 0 or elementName.find('LabelButton_') >= 0:
            ...
            G_CanvasFontDictionary[uiName][elementName].append(
                [newFont, fontName, str(fontSize), fontWeight, fontSlant, str(fontUnderline), str(fontOverstrike)])


def GetFont(uiName, elementName, fontName, fontSize, fontWeight='normal', fontSlant='roman', fontUnderline=0,
            fontOverstrike=0, createifnofind=True):
    PrintFunctionInfo(GetFont,
                      [uiName, elementName, fontName, fontSize, fontWeight, fontSlant, fontUnderline, fontOverstrike,
                       createifnofind])
    # 取得字体
    if type(fontWeight) == type(bool):
    ...
    if uiName in G_UIElementDictionary:
        if elementName in G_UIElementDictionary[uiName]:
            ...
            if elementName not in G_UIRadioButtonGroupArray[uiName][GroupName]:
                G_UIRadioButtonGroupArray[uiName][GroupName][elementName] = [[Control.cget('bg'), Control.cget('fg')],
                                                                             [BGColor, FGColor]]
            else:
    ...
    if G_ComboBoxPopdownWindowSettingEnable == True:
        ...
        if uiName in G_UIElementDictionary:
            if elementName in G_UIElementDictionary[uiName]:
                Control = G_UIElementDictionary[uiName][elementName]
                try:
                    Control.tk.eval('[ttk::combobox::PopdownWindow %s].f.l configure -foreground %s -background %s' % (
                    Control, FGColor, BGColor))
                except:
        ...
        if realElementName.find('Form_') >= 0 or realElementName.find('Canvas_') >= 0:
            ...
            if realElementName.find('Form_') >= 0:
                if Control_Width == 1 and Control_Height == 1:
                    ...
                    elif isinstance(Root.master, tkinter.Frame) == True or isinstance(Root.master,
                                                                                      tkinter.LabelFrame) == True:
            ...
            if wrapType == "zoom":
                ...
                for r in range(RepeatRow):
                    for c in range(RepeatCow):
                        Control.create_image(c * newImage.width, r * newImage.height, anchor=tkinter.NW,
                                             image=newPTImage, tag="BGImage")
            else:
                newPTImage = ImageTk.PhotoImage(newImage)
                AddUserData(uiName, elementName, 'BGImage', 'imageInfo', [newPTImage, imagePath, wrapType], 0)
                Control.create_image(0, 0, anchor=tkinter.NW, image=newPTImage, tag="BGImage")
            ...
            try:
                if url in g_DownLoadImageDictionary:
                    if ControlType == "Canvas":
                        Control.delete('BGImage')
                        Control.create_image(0, 0, anchor=tkinter.NW, image=g_DownLoadImageDictionary[url],
                                             tag="BGImage")
...
def LoadImageFromFile(imagefile, imageSize=None, uiName=None, elementName=None):
    ...
    if imagefile != None:
        ...
        try:
            ...
            if uiName and elementName:
                realElementName = elementName
                if uiName in G_UIElementAliasDictionary.keys() and elementName in G_UIElementAliasDictionary[
                    uiName].keys():
                    realElementName = G_UIElementAliasDictionary[uiName][elementName]
                newPTImage = ImageTk.PhotoImage(newImage)
                if realElementName.find('Form_') >= 0 or realElementName.find('Canvas_') >= 0:
                    ...
                    AddUserData(uiName, elementName, 'image', 'imageInfo', [[newPTImage, resourPath, False, 'normal']],
                                0)
            return newImage
        except Exception as ex:
...
def LoadGIF(uiName, elementName, imagefile, width=0, height=0):
    ...
    if Control:
        if uiName in G_UIElementAliasDictionary.keys() and elementName in G_UIElementAliasDictionary[uiName].keys():
            elementName = G_UIElementAliasDictionary[uiName][elementName]
        if elementName.find('Label_') >= 0 or elementName.find('Button_') >= 0 or elementName.find(
                'RadioButton_') >= 0 or elementName.find('CheckButton_') >= 0 or elementName.find('Text_') >= 0:
                ...
                if os.path.exists(resourPath) == True:
                    try:
                        if imagefile.find('.gif') >= 0:
                            ...
                            G_CanvasImageDictionary[uiName][elementName].append(
                                [imagefile, [seq, delay, 0, None], width, height])
                        else:
                            newImage = Image.open(resourPath).convert('RGBA')
                    except:
                        return newImage
...
def AddPage(uiName, elementName, title="", iconFile="", importUI=''):
    ...
    if len(iconFile) > 0 and os.path.exists(iconFile) == True:
        ...
        NoteBook.add(PageFrame, text=title, image=G_UIElementIconDictionary[uiName][elementName][title],
                     compound="left")
    else:
...
def AddTreeItem(uiName, elementName, parentItem="", insertItemPosition="end", itemName="", itemText="", itemValues=(),
                iconName="", tag=""):
    PrintFunctionInfo(AddTreeItem,
                      [uiName, elementName, parentItem, insertItemPosition, itemName, itemText, itemValues, iconName,
                       tag])
            ...
            if elementName.find('TreeView_') >= 0 and len(itemName) > 0:
                ...
                if iconName != "":
                    if iconName in G_UIElementIconDictionary[uiName][elementName]:
                        ...
                        Item = G_UIElementDictionary[uiName][elementName].insert(parentItem, insertItemPosition,
                                                                                 itemName, text=itemText,
                                                                                 values=itemValues, image=ItemIcon,
                                                                                 tag=tag)
                    else:
                        ItemIcon = None
                        ...
                        if ItemIcon:
                            ...
                            Item = G_UIElementDictionary[uiName][elementName].insert(parentItem, insertItemPosition,
                                                                                     itemName, text=itemText,
                                                                                     values=itemValues, tag=tag)
                else:
                    Item = G_UIElementDictionary[uiName][elementName].insert(parentItem, insertItemPosition, itemName,
                                                                             text=itemText, values=itemValues, tag=tag)
    ...
    if uiName in G_UIElementDictionary:
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('TreeView_') >= 0 and len(itemName) > 0:
                ...
                if item_value:
                    if type(item_value) == tuple or type(item_value) == list:
                        return item_value[0]
                    return item_value
    return None
        ...
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('ListView_') >= 0:
                ...
                for columnName in columnList:
                    G_UIElementDictionary[uiName][elementName].column(columnName, anchor='center', width=100,
                                                                      stretch=True)
                    if elementName in G_ListViewCommandDictionary[uiName]:
                        ...
                        G_UIElementDictionary[uiName][elementName].heading(columnName, anchor='center', text=columnName,
                                                                           command=partial(
                                                                               ListViewHeadingFunction_Adaptor,
                                                                               fun=eventFunc, uiName=uiName,
                                                                               widgetName=elementName,
                                                                               columnname=columnName))
                    else:
    ...
    if uiName in G_UIElementDictionary:
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('ListView_') >= 0:
                ...
                for columnName in columnList:
                    G_UIElementDictionary[uiName][elementName].column(columnName, anchor=anchor, width=width,
                                                                      stretch=stretch)
                    if elementName in G_ListViewCommandDictionary[uiName]:
                        eventFunc = G_ListViewCommandDictionary[uiName][elementName]
                        G_UIElementDictionary[uiName][elementName].heading(columnName, anchor='center', text=columnName,
    ...
    if uiName in G_UIElementDictionary:
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('ListView_') >= 0:
                ...
                # 获取Treeview的Tcl路径
                # tree_tcl_path = ListView.winfo_pathname(ListView.winfo_id())
                tree_tcl_path = ListView._w
                # 生成带标签的Tcl脚本
                tcl_script = []
                is_mainthread = CurrentThreadIsMainThread()
                for rowOffset in range(rowCount):
                    ...
                    if rowOffset < tagCount:
                        ...
                        if (currentRowIndex + rowOffset) % 2 == 0:
                            ...
                            tag = 'odd'
                    if is_mainthread == False:
                        ListView.insert('', rowIndex, values=values_list, tag=tag)
                    else:
                        valuesText = ''
                        IncludeSpecChar = False
                        for i in range(len(values_list)):
                            itemText = str(values_list[i])
                            if itemText.find(',') >= 0 or itemText.find('{') >= 0 or itemText.find('[') >= 0:
                                ...
                                if i < len(values_list) - 1:
                                    valuesText += ' '
                        if IncludeSpecChar == False:
                            cmd = f"{tree_tcl_path} insert {{}} end -values {{{valuesText}}} -tag {{{tag}}}"
                            tcl_script.append(cmd)
                        else:
                            if tcl_script:
                                full_script = "\n".join(tcl_script)
                                ListView.tk.eval(full_script)
                            ListView.insert('', rowIndex, values=values_list, tag=tag)
                            tcl_script = []

                if is_mainthread == True and len(tcl_script) > 0:
                    full_script = "\n".join(tcl_script)
                    # 测试性能
                    # start = time.time()
                    ListView.tk.eval(full_script)
                    # end = time.time()
                    # print(f"插入耗时: {end - start:.4f}秒")
                return currentRowIndex
        ...
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('ListView_') >= 0:
                ...
                if CellLabelInfoList:
                    ...
                    AddUserData(uiName, elementName, 'CellLabelInfo', 'object',
                                [[rowIndex, columnIndex, CellLabel, PTImage]], 0)
...
def SetCellEntry(uiName, elementName, rowIndex, columnIndex, callback=None):
    ...
    if uiName in G_UIElementDictionary:
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('ListView_') >= 0:
                ListView = G_UIElementDictionary[uiName][elementName]
                if rowIndex >= len(ListView.get_children()):
                    return
                RowItem = ListView.get_children()[rowIndex]
                if columnIndex >= len(ListView.item(RowItem)["values"]):
                    return
                inittext = ListView.item(RowItem)["values"][columnIndex]
                # 取得对应行和列的位置和大小
                CellBox = ListView.bbox(RowItem, columnIndex)
                ...
                if CellEntryInfoList:
                    ...
                    AddUserData(uiName, elementName, 'CellEntryInfo', 'object',
                                [[rowIndex, columnIndex, CellEntry, CellEntry_TextVariable]], 0)
        ...
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('ListView_') >= 0:
                ...
                if CellComboBoxInfoList:
                    ...
                    AddUserData(uiName, elementName, 'CellComboBoxInfo', 'object',
                                [[rowIndex, columnIndex, CellComboBox, CellComboBox_TextVariable]], 0)
    ...
    if uiName in G_UIElementDictionary:
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('ListView_') >= 0:
                ...
                ChildCount = len(RootChildren)
                is_mainthread = CurrentThreadIsMainThread()
                if is_mainthread == True:
                    # 获取 Treeview 的 Tcl 路径（例如 .!treeview）
                    tree_tcl_path = ListView._w  # 或 tree.winfo_pathname(tree.winfo_id())
                    # 生成 Tcl 脚本（插入数据时附加标签）
                    tcl_script = []
                    for row in range(rowIndex, ChildCount):
                        Item = RootChildren[row]
                        ItemInfo = ListView.item(Item)
                        if 'tags' in ItemInfo:
                            if ItemInfo['tags'][0] == "even" or ItemInfo['tags'][0] == "odd":
                                rowTag = "even" if row % 2 == 0 else "odd"
                                cmd = f"{tree_tcl_path} item {Item} -tag {rowTag}"
                                tcl_script.append(cmd)
                    full_script = "\n".join(tcl_script)
                    ListView.tk.eval(full_script)
                else:
                    for row in range(rowIndex, ChildCount):
                        ...
                        if 'tags' in ItemInfo:
                            if ItemInfo['tags'][0] == "even" or ItemInfo['tags'][0] == "odd":
                                rowTag = "even" if row % 2 == 0 else "odd"
                                ListView.item(Item, tags=rowTag)
    CellLabelInfoList = GetUserData(uiName, elementName, 'CellLabelInfo')
    if CellLabelInfoList:
        for CellLabelInfo in CellLabelInfoList:
            if CellLabelInfo[0] == rowIndex:
                CellLabelInfo[0].destroy()
                CellLabelInfoList.remove(CellLabelInfo)
                for CellLabelInfo in CellLabelInfoList:
                    if CellLabelInfo[0] > rowIndex:
                        CellLabelInfo[0] -= 1
                break
    ...
    if CellComboBoxInfoList:
        for CellComboBoxInfo in CellComboBoxInfoList:
            if CellComboBoxInfo[0] == rowIndex:
                ...
                for CellComboBoxInfo in CellComboBoxInfoList:
                    if CellComboBoxInfo[0] > rowIndex:
                        CellComboBoxInfo[0] -= 1
                break
        SetUserData(uiName, elementName, 'CellComboBoxInfo', CellComboBoxInfoList)


def DeleteRows(uiName, elementName, rowIndexList):
    PrintFunctionInfo(DeleteRow, [uiName, elementName, rowIndexList])
    # 删除ListView指定行
            ...
            if elementName.find('ListView_') >= 0:
                ...
                if is_mainthread == True:
                    ...
                    for rowIndex in rowIndexList:
                        ...
                        cmd = f"{tree_tcl_path} delete {Item}"
                        tcl_script.append(cmd)
                    full_script = "\n".join(tcl_script)
                    ...
                    RootChildren = ListView.get_children()
                    ChildCount = len(RootChildren)
                    minRowIndex = min(rowIndexList)
                    # 生成 Tcl 脚本（插入数据时附加标签）
                    tcl_script = []
                    for row in range(minRowIndex, ChildCount):
                        Item = RootChildren[row]
                        ItemInfo = ListView.item(Item)
                        if 'tags' in ItemInfo:
                            if ItemInfo['tags'][0] == "even" or ItemInfo['tags'][0] == "odd":
                    ...
                    rowHandleList = []
                    for rowIndex in rowIndexList:
                        rowHandleList.append(G_UIElementDictionary[uiName][elementName].get_children()[rowIndex])
                    for rowHandle in rowHandleList:
                        G_UIElementDictionary[uiName][elementName].delete(rowHandle)
    CellLabelInfoList = GetUserData(uiName, elementName, 'CellLabelInfo')
    if CellLabelInfoList:
...
def SortListView(uiName, elementName, columnIndex=0, reverse=False, sortkeyDict={}):
    PrintFunctionInfo(SortListView, [uiName, elementName, columnIndex, sortkeyDict, reverse])
    # 设置ListView按指定列排序
        ...
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('ListView_') >= 0:
                ...
                for Item in RootChildren:
                    itemInfo = ListView.item(Item)
                    AllItemList.append(itemInfo)
                    ColumnText = itemInfo["values"][columnIndex]
                    SortValue = 0
                    if sortkeyDict:
                        if ColumnText in sortkeyDict.keys():
                            SortValue = sortkeyDict[ColumnText]
                        else:
                            for keyName in sortkeyDict.keys():
                                if ColumnText.find(keyName) >= 0:
                                    SortValue = sortkeyDict[keyName]
                                    break
                    else:
                        SortValue = int(ColumnText)
                    SortLineValues.append([LineIndex, SortValue])
                    LineIndex = LineIndex + 1

                # 让AllItemList按照第columnIndex列的文本值基于sortkeyDict字典排序
                if type(SortLineValues[1]) == int:
                    SortLineValues.sort(key=lambda x: int(x[1]), reverse=reverse)
                elif type(SortLineValues[1]) == float:
                    SortLineValues.sort(key=lambda x: float(x[1]), reverse=reverse)
                else:
                    SortLineValues.sort(key=lambda x: str(x[1]), reverse=reverse)
                AllItem_Sort = []
                for SortLine in SortLineValues:
                    AllItem_Sort.append(AllItemList[SortLine[0]])
                ListView.delete(*ListView.get_children())
                is_mainthread = CurrentThreadIsMainThread()
                if is_mainthread == True:
                    # 获取 Treeview 的 Tcl 路径
                    tree_tcl_path = ListView._w
                    # 生成带标签的Tcl脚本
                    tcl_script = []
                    for ItemInfo in AllItemList:
                        lineText = ' '
                        for item in ItemInfo["values"]:
                            if item == '':
                                lineText = lineText + ' " "'
                            else:
                                lineText = lineText + ' "' + str(item) + '"'
                        if lineText.find(',') >= 0 or lineText.find('{') >= 0 or lineText.find('[') >= 0:
                            if tcl_script:
                                full_script = "\n".join(tcl_script)
                                ListView.tk.eval(full_script)
                            ListView.insert("", "end", values=ItemInfo["values"], tags=ItemInfo["tags"])
                            tcl_script = []
                        else:
                            tag = ItemInfo["tags"][0]
                            cmd = f"{tree_tcl_path} insert {{}} end -values {{{lineText}}} -tag {{{tag}}}"
                            tcl_script.append(cmd)
                    if tcl_script:
                        full_script = "\n".join(tcl_script)
                        ListView.tk.eval(full_script)
        ...
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('ListView_') >= 0:
                ...
                for Item in RootChildren:
                    itemInfo = ListView.item(Item)
                    AllItemList.append(itemInfo)
                if type(AllItemList[0]["values"][columnIndex]) == int:
                    AllItemList.sort(key=lambda x: int(x["values"][columnIndex]), reverse=reverse)
                elif type(AllItemList[0]["values"][columnIndex]) == float:
                    AllItemList.sort(key=lambda x: float(x["values"][columnIndex]), reverse=reverse)
                else:
                    AllItemList.sort(key=lambda x: str(x["values"][columnIndex]), reverse=reverse)
                ListView.delete(*ListView.get_children())
                is_mainthread = CurrentThreadIsMainThread()
                if is_mainthread == True:
                    # 获取 Treeview 的 Tcl 路径
        ...
        if elementName in G_UIElementDictionary[uiName]:
            if elementName.find('ListView_') >= 0:
                ...
                if rowIndex == 'even':
                    ...
                    if textFont:
                        ListView.tag_configure(str('row_%d' % rowIndex), background=bgColor, font=textFont,
                                               foreground=fgColor)
                    else:
                    ...
                    if RowIndex >= 0:
                        ...
                        if HoverItem:
                            ...
                            if HoverItem:
                                ...
                                AddUserData(uiName, elementName, 'HoverItem', 'list', [NewItem, NewItemTag, LastItemBG],
                                            0)
    return True
...
def ModifyItemValue(uiName, elementName, index, value):
    PrintFunctionInfo(ModifyItemValue, [uiName, elementName, index, value])
    # 设置ListBox、ComboBox、SpinBox_的指定项的文本。参数1 :界面类名, 参数2:控件名称, 参数3:对应的索引，参数4：要设置的文本。
    ValueList = GetValueList(uiName, elementName)
    if ValueList:
        ...
        SetValueList(uiName, elementName, ValueList)
...
def MovingScrollXOffset(uiName, elementName, step=0.1):
    PrintFunctionInfo(MovingScrollXOffset, [uiName, elementName, step])
    # 滚动条横向移动指定步长:参数1 :界面类名, 参数2:控件名称, 参数3:滚动条移动步长值
    Control = GetElement(uiName, elementName)
    if Control:
        if uiName in G_UIElementAliasDictionary.keys() and elementName in G_UIElementAliasDictionary[uiName].keys():
            elementName = G_UIElementAliasDictionary[uiName][elementName]
        if elementName.find("Text_") >= 0:
            Control.MovingScrollXOffset(step)
        else:
            Control.xview("scroll", step, "units")


def MovingScrollYOffset(uiName, elementName, step=0.1):
    PrintFunctionInfo(MovingScrollYOffset, [uiName, elementName, step])
    # 滚动条纵向移动指定步长:参数1 :界面类名, 参数2:控件名称, 参数3:滚动条移动步长值
    Control = GetElement(uiName, elementName)
    if Control:
        if uiName in G_UIElementAliasDictionary.keys() and elementName in G_UIElementAliasDictionary[uiName].keys():
            elementName = G_UIElementAliasDictionary[uiName][elementName]
        if elementName.find("Text_") >= 0:
            Control.MovingScrollYOffset(step)
        else:
            Control.yview("scroll", step, "units")


def MovingScrollXTo(uiName, elementName, x=1.0):
    PrintFunctionInfo(MovingScrollXTo, [uiName, elementName, x])
    # 滚动条横向移动到目标位置:参数1 :界面类名, 参数2:控件名称, 参数3:滚动条目标位置
    Control = GetElement(uiName, elementName)
    if Control:
        if uiName in G_UIElementAliasDictionary.keys() and elementName in G_UIElementAliasDictionary[uiName].keys():
            elementName = G_UIElementAliasDictionary[uiName][elementName]
        if elementName.find("Text_") >= 0:
            Control.MovingScrollXTo(x)
        else:
            Control.xview_moveto(x)


def MovingScrollYTo(uiName, elementName, y=1.0):
    PrintFunctionInfo(MovingScrollYTo, [uiName, elementName, y])
    # 滚动条纵向移动到目标位置:参数1 :界面类名, 参数2:控件名称, 参数3:滚动条目标位置
    Control = GetElement(uiName, elementName)
    if Control:
        if uiName in G_UIElementAliasDictionary.keys() and elementName in G_UIElementAliasDictionary[uiName].keys():
            elementName = G_UIElementAliasDictionary[uiName][elementName]
        if elementName.find("Text_") >= 0:
            Control.MovingScrollYTo(y)
        else:
            Control.yview_moveto(y)
...
def InitElementData(uiName):
    ...
    if uiClass:
        ...
        if G_WindowDraggable:
            uiRoot = GetElement(uiName, "root")
            for WindowInstance in G_WindowDraggable:
                if uiRoot is WindowInstance.GetWidget():
                    for formName in uiRoot.children.keys():
                        formwidget = uiRoot.children[formName]
                        IsTopLevel = False
                        for WinID in G_TopLevelDict.keys():
                            if formwidget is G_TopLevelDict[WinID][1]:
                                IsTopLevel = True
                                break
                        if IsTopLevel == True:
                            continue
                        formwidget.bind('<ButtonPress-1>', WindowInstance.StartDrag, add=True)
                        formwidget.bind('<ButtonRelease-1>', WindowInstance.StopDrag, add=True)
                        formwidget.bind('<B1-Motion>', WindowInstance.MoveDragPos, add=True)
                        for childName in formwidget.children.keys():
                            childwidget = formwidget.children[childName]
                            if childwidget.winfo_class() == "Label" or childwidget.winfo_class() == "Message" or childwidget.winfo_class() == "Canvas" or childwidget.winfo_class() == "Labelframe" or childwidget.winfo_class() == "Frame":
                                ...
                                if bindingFunc:
                                    continue
                                IsSliderCanvas = False
                                for elementName in G_UIElementDictionary[uiName].keys():
                                    if elementName.find("Slider_") >= 0:
                                        Widget = G_UIElementDictionary[uiName][elementName]
                                        SliderCanvas = Widget.GetWidget()
                                        if SliderCanvas == childwidget:
                                            IsSliderCanvas = True
                                            break
                                if IsSliderCanvas == True:
                                    continue
                                childwidget.bind('<ButtonPress-1>', WindowInstance.StartDrag, add=True)
                                childwidget.bind('<ButtonRelease-1>', WindowInstance.StopDrag, add=True)
                                childwidget.bind('<B1-Motion>', WindowInstance.MoveDragPos, add=True)
...
def GetUIDataDictionary(uiName):
    ...
    if uiName not in G_UIElementDictionary:
        ...
        for elementName in G_UIElementDictionary[uiName].keys():
            ...
            Widget = G_UIElementDictionary[uiName][elementName]
            if elementName.find('_ParentCanvas') >= 0:
                continue
    ...
    if root:
        ...
        try:
            ...
            if hasattr(importModule, targetUIName) == True:
                ...
                if params is None:
                    uiDialog = uiClass(root, True)
                else:
                    try:
                        uiDialog = uiClass(root, True, params)
                    except Exception as ex:
                        uiDialog = uiClass(root, True)
                if hasattr(importModule, "Fun") == True:
                    try:
...
def PlayCallUIDialogAction(topLevel, uiInstance, animation='zoomin'):
    ...
    def ZoomIn(topLevel, uiInstance, zoom, width, height):
        try:
            ...
            pass
        if zoom < 1.0:
            topLevel.after(1, lambda: ZoomIn(topLevel=topLevel, uiInstance=uiInstance, zoom=zoom, width=width,
                                             height=height))
    ...
    if animation == "fadein":
        ...
        try:
            ...
            topLevel.deiconify()
            topLevel.after(1, lambda: ZoomIn(topLevel=topLevel, uiInstance=uiInstance, zoom=0.0, width=form1_width,
                                             height=form1_height))
        except ImportError:
            pass
        ...
        if hasattr(importModule, uiName) == True and hasattr(importModule, "Fun") == True:
            ...
            try:
                ...
                if wait_window == 1:
                    ...
                    WinID = topLevel.winfo_id()
                    if WinID in G_TopLevelDict.keys():
                        while G_TopLevelDict[WinID][0] == True:
                            topLevel.update()
                            G_TKRoot.update()
                            # G_TKRoot.after(100)
                    print("窗口已关闭")
...
def LoadUIDialog(uiName, elementName, targetUIName, params=None, ignoreSameUI=True):
    ...
    if currUIDialog is None:
        ...
        # 调用Form_1_onExit
        currUIName = currUIDialog.replace(".py", "")
        Form1 = GetElement(currUIName, 'Form_1')
        if Form1:
            UIClass = GetElement(currUIName, 'UIClass')
            if UIClass:
                Temp_isTKroot = UIClass.isTKroot
                UIClass.isTKroot = False
                if hasattr(UIClass, "Exit") == True:
    ...
    if targetUIName:
        ...
        try:
            ...
            try:
                ...
                except_file = os.path.split(except_stack_end.f_code.co_filename)[1]
                MessageBox(
                    '错误信息：' + except_value_str + '\n' + '文件:' + except_file + '\n' + '行号:' + except_stack_lineno,
                    '运行错误')
        except Exception as ex:
            print("Exception %s" % (ex))
            except_type, except_value, except_traceback = sys.exc_info()
            ...
            if ParentFrame_Child:
                if hasattr(uiDialog, "GetRootSize") == True:
                    ...
                    if 'Form_1' in ChildWidgetList.keys():
                        ...
                        ChildHandle = ParentFrame.create_window(x, y, window=uiDialogForm1, anchor=tkinter.NW,
                                                                tag="Form_1")
...
def CenterDlg(uiName, popupDlg, dw=0, dh=0, keepHide=False, popui_xy='Center'):
    ...
    if root != None and popupDlg != root:
        ...
        if popui_xy == 'Center':
            ...
            x = popui_xy[0]
            y = popui_xy[1]
        popupDlg.tk.call('tk', 'scaling', g_TKScaling_Init)
...
def RestoreUI(uiName):
    ...
    if root:
        if 'root' in G_UIElementPlaceDictionary[uiName]:
            ...
            G_UIElementPlaceDictionary[uiName]['root']['width'], G_UIElementPlaceDictionary[uiName]['root']['height'],
            G_UIElementPlaceDictionary[uiName]['root']['x'], G_UIElementPlaceDictionary[uiName]['root']['y']))
        else:
            hwnd = windll.user32.GetParent(root.winfo_id())
            state = 'normal'
...
def MessageBox(text="", title="info", type="info", parent=None):
    ...
    G_TKRoot.after(10, ResetMessageBox)
...
def RunApplication(uiClass, deiconify=False, projName='', InitCheckFunc=None):
    ...
    try:
        ...
        except_file = os.path.split(except_stack_end.f_code.co_filename)[1]
        MessageBox(
            '错误信息：' + except_value_str + '\n' + '文件:' + except_file + '\n' + '行号:' + str(except_stack_lineno),
            '运行错误')


def ExitApplication():
    PrintFunctionInfo(ExitApplication, [])
    # 退出程序
    global G_TKRoot
    G_TKRoot.quit()
    os._exit(0)
...
def InputDialog(width, lines=1, bgColor='#f8f9fa', titleText='', promptText='', defaultText='', callBackFunction=None):
    ...
    theDataLabel = tkinter.Label(theForm, anchor=tkinter.W, text=promptText, width=width, bg=bgColor, fg='#000000',
                                 font=('宋体', 12), justify=tkinter.LEFT, height=1)
    theDataLabel.place(x=20, y=10, width=width - 40, height=30)
    theYPos = 75
    theEntryText = StringVar()
    theEntryText.set(defaultText)
    if lines == 1:
        ...
        theEntry.insert(tkinter.END, defaultText)
        theYPos = 45 + lines * 20
    ...
    theCancelButton = tkinter.Button(theForm, anchor=tkinter.CENTER, text='取消', width=100, height=1,
                                     command=closeDialog)
...
def SetControlPlace(uiName, elementName, x, y, w, h, anchorpoint='nw', visible=True, modify=True):
    ...
    if Control != None:
        ...
        try:
            ...
            if len(PlaceInfo) > 0:
                # 避免拖动窗体时闪烁
                if ("relx" in PlaceInfo and float(PlaceInfo["relx"]) > 0) or (
                        "rely" in PlaceInfo and float(PlaceInfo["rely"]) > 0):
                    Control.place_forget()
        except Exception as ex:
            # 对于某些非界面组件也调用了，就直接返回
            return
    ...
    if Control != None and visible == True:
        Control.update()
        if elementName.find("Frame_") >= 0 or elementName.find("LabelFrame_") >= 0 or elementName.find(
                "PanedWindow_") >= 0:
                        ...
                        if "UIClass" in G_UIElementDictionary[uiName].keys():
                            ...
                            if uiClass:
                                ...
                                if Form_1:
                                    ...
                                    if hasattr(uiClass, "Configure") == True:
                                        uiClass.Configure(event)
    if elementName.find("LabelButton_") >= 0 or elementName.find("Entry") >= 0 or elementName.find(
            "Text") >= 0 or elementName.find("SwitchButton") >= 0:
        if hasattr(OldControl, "Configure") == True:
            event = ChartEvent(w, h, OldControl)
            OldControl.Configure(event)
    if elementName.find("Calendar_") >= 0 or elementName.find("DatePicker_") >= 0 or elementName.find(
            "Navigation_") >= 0 or elementName.find("ListMenu_") >= 0 or elementName.find(
            "SwitchPage_") >= 0 or elementName.find("ShowCase_") >= 0:
        if visible == False:
...
def UpdateElementPlace(uiName, elementName, HScrollBarOffsetY=0, VScrollBarOffsetX=0):
    ...
    if elementName in G_UIElementPlaceDictionary[uiName]:
        ...
        if isinstance(ControlParentWidget, tkinter.Canvas) == True:
            uiName2, parentElementName = GetElementName(ControlParentWidget)
            if parentElementName == "Form_1":
                # if ParentWidth == 1 and ParentHeight == 1:
                if uiName2 in G_UIRootSizeDictionary.keys():
                    ParentWidth = G_UIRootSizeDictionary[uiName2]["width"]
                    ParentHeight = G_UIRootSizeDictionary[uiName2]["height"]
                else:
                    ParentWidth = ControlParentWidget.winfo_width()
                    ParentHeight = ControlParentWidget.winfo_height()
            else:
            ...
            scrollregion = ControlParentWidget.cget("scrollregion")
            if scrollregion:
                return
        if ParentWidth == 1 and ParentHeight == 1:
            return
        ...
        if Visible == True:
            ...
            #     else:
            #         if VScrollbar:
            #             HScrollbar.place(x = 0,y = Height_PX-20+HScrollBarOffsetY,width = Width_PX-20,height = 20)
            #         else:
            ...
            #     else:
            #         if HScrollbar:
            #             VScrollbar.place(x = Width_PX-20+VScrollBarOffsetX,y = 0,width = 20,height = Height_PX-20)
            #         else:
...
def ResizeRoot(uiName, root, event):
    ...
    if isinstance(root, tkinter.Frame) == True or isinstance(root, tkinter.LabelFrame) == True or isinstance(root,
                                                                                                             tkinter.ttk.Frame) == True:
...
def DoCanvasRecord(uiName, drawCanvasName, shapeType, x, y, x2, y2, Anchor, fillcolor, outlinecolor, fillwidth, dash1=0,
                   dash2=0, newImage=None, text='', textFont=None, textColor='', shapeTag=''):
    PrintFunctionInfo(DoCanvasRecord,
                      [uiName, drawCanvasName, shapeType, x, y, x2, y2, Anchor, fillcolor, outlinecolor, fillwidth,
                       dash1, dash2, newImage, text, textFont, textColor, shapeTag])
    # 画板动作处理函数
    ...
    if drawCanvas != None:
        ...
        if shapeType == 'line' or shapeType == 'pen':
            if dash1 > 0:
                ...
                if shapeTag in G_CanvasParamDictionary[uiName][drawCanvasName].keys():
                    if G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag][5]:
                        ...
                        draw.line(
                            (x - left + fillwidth, y - top + fillwidth, x2 - left + fillwidth, y2 - top + fillwidth), p)
                        draw.flush()
                        newImage = ImageTk.PhotoImage(img)
                        G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag][5] = newImage
                        drawCanvas.create_image(left - fillwidth, top - fillwidth, image=newImage, anchor='nw',
                                                tag=shapeTag)
                    else:
            ...
            if dash1 > 0:
                drawCanvas.create_line(x, y, x2, y2, arrow=tkinter.LAST, fill=fillcolor, dash=(dash1, dash2),
                                       width=fillwidth, tag=shapeTag)
            else:
            ...
            if fillcolor.find('#') < 0:
                if dash1 > 0:
                    drawCanvas.create_rectangle(x, y, x2, y2, outline=outlinecolor, dash=(dash1, dash2),
                                                width=fillwidth, tag=shapeTag)
                else:
                    drawCanvas.create_rectangle(x, y, x2, y2, outline=outlinecolor, width=fillwidth, tag=shapeTag)
            else:
                if dash1 > 0:
                    drawCanvas.create_rectangle(x, y, x2, y2, fill=fillcolor, outline=outlinecolor, dash=(dash1, dash2),
                                                width=fillwidth, tag=shapeTag)
                else:
                    drawCanvas.create_rectangle(x, y, x2, y2, fill=fillcolor, outline=outlinecolor, width=fillwidth,
                                                tag=shapeTag)
            ...
            if roundRadius == 0:
                if fillcolor.find('#') < 0:
                    if dash1 > 0:
                        drawCanvas.create_rectangle(x, y, x2, y2, fill='', outline=outlinecolor, dash=(dash1, dash2),
                                                    width=fillwidth, tag=shapeTag)
                    else:
                        drawCanvas.create_rectangle(x, y, x + width, y + height, fill='', outline=outlinecolor,
                                                    width=fillwidth, tag=shapeTag)
                else:
                    if dash1 > 0:
                        drawCanvas.create_rectangle(x, y, x2, y2, fill=fillcolor, outline=outlinecolor,
                                                    dash=(dash1, dash2), width=fillwidth, tag=shapeTag)
                    else:
                        drawCanvas.create_rectangle(x, y, x + width, y + height, fill=fillcolor, outline=outlinecolor,
                                                    width=fillwidth, tag=shapeTag)
            else:
                if fillcolor.find('#') == 0:
                    drawCanvas.create_rectangle(x + roundRadius, y + roundRadius, x + width - roundRadius,
                                                y + height - roundRadius, fill=fillcolor, width=0, tag=shapeTag)
                    drawCanvas.create_rectangle(x + roundRadius, y, x + width - roundRadius, y + roundRadius,
                                                fill=fillcolor, width=0, tag=shapeTag)
                    drawCanvas.create_rectangle(x + roundRadius, y + height - roundRadius, x + width - roundRadius,
                                                y + height, fill=fillcolor, width=0, tag=shapeTag)
                    drawCanvas.create_rectangle(x, y + roundRadius, x + roundRadius, y + height - roundRadius,
                                                fill=fillcolor, width=0, tag=shapeTag)
                    drawCanvas.create_rectangle(x + width - roundRadius, y + roundRadius, x + width,
                                                y + height - roundRadius, fill=fillcolor, width=0, tag=shapeTag)
            OutLineTag = shapeTag + "_outline"
            if fillwidth > 0:
                if dash1 > 0:
                    drawCanvas.create_line(x + roundRadius, y, x + width - roundRadius, y, fill=outlinecolor,
                                           dash=(dash1, dash2), tag=OutLineTag, width=fillwidth)
                    drawCanvas.create_line(x + roundRadius, y + height, x + width - roundRadius, y + height,
                                           fill=outlinecolor, dash=(dash1, dash2), tag=OutLineTag, width=fillwidth)
                    drawCanvas.create_line(x, y + roundRadius, x, y + height - roundRadius, fill=outlinecolor,
                                           tag=OutLineTag, dash=(dash1, dash2), width=fillwidth)
                    drawCanvas.create_line(x + width, y + roundRadius, x + width, y + height - roundRadius,
                                           fill=outlinecolor, dash=(dash1, dash2), tag=OutLineTag, width=fillwidth)
                else:
                    drawCanvas.create_line(x + roundRadius, y, x + width - roundRadius, y, fill=outlinecolor,
                                           tag=OutLineTag, width=fillwidth)
                    drawCanvas.create_line(x + roundRadius, y + height, x + width - roundRadius, y + height,
                                           fill=outlinecolor, tag=OutLineTag, width=fillwidth)
                    drawCanvas.create_line(x, y + roundRadius, x, y + height - roundRadius, fill=outlinecolor,
                                           tag=OutLineTag, width=fillwidth)
                    drawCanvas.create_line(x + width, y + roundRadius, x + width, y + height - roundRadius,
                                           fill=outlinecolor, tag=OutLineTag, width=fillwidth)
            if fillcolor.find('#') == 0:
                drawCanvas.create_arc(x, y, x + 2 * roundRadius, y + 2 * roundRadius, start=180, extent=-90,
                                      fill=fillcolor, outline=fillcolor, width=0, tag=shapeTag)
                drawCanvas.create_arc(x + width - 2 * roundRadius, y, x + width, y + 2 * roundRadius, extent=90,
                                      fill=fillcolor, outline=fillcolor, width=0, tag=shapeTag)
                drawCanvas.create_arc(x + width - 2 * roundRadius, y + height - 2 * roundRadius, x + width, y + height,
                                      extent=-90, fill=fillcolor, outline=fillcolor, width=0, tag=shapeTag)
                drawCanvas.create_arc(x, y + height - 2 * roundRadius, x + 2 * roundRadius, y + height, start=180,
                                      extent=90, fill=fillcolor, outline=fillcolor, width=0, tag=shapeTag)
            OutArcTag = shapeTag + "_arc"
            if fillwidth > 0:
                if dash1 > 0:
                    drawCanvas.create_arc(x, y, x + 2 * roundRadius, y + 2 * roundRadius, start=180, extent=-90,
                                          outline=outlinecolor, dash=(dash1, dash2), width=fillwidth, style='arc',
                                          tag=OutArcTag)
                    drawCanvas.create_arc(x + width - 2 * roundRadius, y, x + width, y + 2 * roundRadius, extent=90,
                                          outline=outlinecolor, dash=(dash1, dash2), width=fillwidth, style='arc',
                                          tag=OutArcTag)
                    drawCanvas.create_arc(x + width - 2 * roundRadius, y + height - 2 * roundRadius, x + width,
                                          y + height, extent=-90, outline=outlinecolor, dash=(dash1, dash2),
                                          width=fillwidth, style='arc', tag=OutArcTag)
                    drawCanvas.create_arc(x, y + height - 2 * roundRadius, x + 2 * roundRadius, y + height, start=180,
                                          extent=90, outline=outlinecolor, dash=(dash1, dash2), width=fillwidth,
                                          style='arc', tag=OutArcTag)
                else:
                    drawCanvas.create_arc(x, y, x + 2 * roundRadius, y + 2 * roundRadius, start=180, extent=-90,
                                          outline=outlinecolor, width=fillwidth, style='arc', tag=OutArcTag)
                    drawCanvas.create_arc(x + width - 2 * roundRadius, y, x + width, y + 2 * roundRadius, extent=90,
                                          outline=outlinecolor, width=fillwidth, style='arc', tag=OutArcTag)
                    drawCanvas.create_arc(x + width - 2 * roundRadius, y + height - 2 * roundRadius, x + width,
                                          y + height, extent=-90, outline=outlinecolor, width=fillwidth, style='arc',
                                          tag=OutArcTag)
                    drawCanvas.create_arc(x, y + height - 2 * roundRadius, x + 2 * roundRadius, y + height, start=180,
                                          extent=90, outline=outlinecolor, width=fillwidth, style='arc', tag=OutArcTag)
        elif shapeType == 'circle':

            if fillcolor.find('#') < 0:
                if dash1 > 0:
                    drawCanvas.create_oval(x, y, x2, y2, outline=outlinecolor, dash=(dash1, dash2), width=fillwidth,
                                           tag=shapeTag)
                else:
            ...
            if fillcolor.find('#') < 0:
                if dash1 > 0:
                    drawCanvas.create_oval(x, y2 - OvalHeight, x2, y2, fill='', outline=outlinecolor,
                                           dash=(dash1, dash2), width=fillwidth, tag=shapeTag)
                    drawCanvas.create_rectangle(x, y + OvalHeight_Half, x2, y2 - OvalHeight_Half, fill='', width=0,
                                                tag=shapeTag)
                    drawCanvas.create_oval(x, y, x2, y + OvalHeight, fill='', outline=outlinecolor, dash=(dash1, dash2),
                                           width=fillwidth, tag=shapeTag)
                    drawCanvas.create_line(x, y + OvalHeight_Half, x, y2 - OvalHeight_Half, fill=outlinecolor,
                                           dash=(dash1, dash2), width=fillwidth, tag=OutLineTag)
                    drawCanvas.create_line(x2, y + OvalHeight_Half, x2, y2 - OvalHeight_Half, fill=outlinecolor,
                                           dash=(dash1, dash2), width=fillwidth, tag=OutLineTag)
                else:
                    drawCanvas.create_oval(x, y2 - OvalHeight, x2, y2, fill='', outline=outlinecolor, width=fillwidth,
                                           tag=shapeTag)
                    drawCanvas.create_rectangle(x, y + OvalHeight_Half, x2, y2 - OvalHeight_Half, fill='', width=0,
                                                tag=shapeTag)
                    drawCanvas.create_oval(x, y, x2, y + OvalHeight, fill='', outline=outlinecolor, width=fillwidth,
                                           tag=shapeTag)
                    drawCanvas.create_line(x, y + OvalHeight_Half, x, y2 - OvalHeight_Half, fill=outlinecolor,
                                           width=fillwidth, tag=OutLineTag)
                    drawCanvas.create_line(x2, y + OvalHeight_Half, x2, y2 - OvalHeight_Half, fill=outlinecolor,
                                           width=fillwidth, tag=OutLineTag)
            else:
                if dash1 > 0:
                    drawCanvas.create_oval(x, y2 - OvalHeight, x2, y2, fill=fillcolor, outline=outlinecolor,
                                           dash=(dash1, dash2), width=fillwidth, tag=shapeTag)
                    drawCanvas.create_rectangle(x, y + OvalHeight_Half, x2, y2 - OvalHeight_Half, fill=fillcolor,
                                                width=0, tag=shapeTag)
                    drawCanvas.create_oval(x, y, x2, y + OvalHeight, fill=fillcolor, outline=outlinecolor,
                                           dash=(dash1, dash2), width=fillwidth, tag=shapeTag)
                    drawCanvas.create_line(x, y + OvalHeight_Half, x, y2 - OvalHeight_Half, fill=outlinecolor,
                                           dash=(dash1, dash2), width=fillwidth, tag=OutLineTag)
                    drawCanvas.create_line(x2, y + OvalHeight_Half, x2, y2 - OvalHeight_Half, fill=outlinecolor,
            ...
            for i in range(rows):
                for j in range(cows):
                    if (i + j) % 2 == 0:
                        tx = x + j * dash1
                        ty = y + i * dash2
                        drawCanvas.create_rectangle(tx, ty, tx + dash1, ty + dash2, fill=fillcolor, width=0,
                                                    tag=shapeTag)
        elif shapeType == 'text':
            drawCanvas.create_text(x, int((y + y2) / 2), fill=fillcolor, text=text, font=textFont, anchor='w',
                                   tag=shapeTag)
            ...
            if newImage:
                ...
                if dash1 > 0:
                    if fillcolor.find('#') < 0:
                        startAngle = 90
                        extentAngle = 180
                        drawCanvas.create_arc(x, y, x + 2 * oval_rx, y2, start=startAngle, extent=extentAngle, fill='',
                                              outline=outlinecolor, dash=(dash1, dash2), style=tkinter.ARC,
                                              width=fillwidth, tag=shapeTag)
                        drawCanvas.create_line(x + oval_rx, y + half_width, x + oval_rx, y2 - half_width,
                                               fill=outlinecolor, dash=(dash1, dash2), width=fillwidth, tag=shapeTag)
                        drawCanvas.create_line(x2 - oval_rx, y + half_width, x2 - oval_rx, y2 - half_width,
                                               fill=outlinecolor, dash=(dash1, dash2), width=fillwidth, tag=shapeTag)
                        startAngle = 90
                        extentAngle = -180
                        drawCanvas.create_arc(x2 - 2 * oval_rx, y, x2, y2, start=startAngle, extent=extentAngle,
                                              fill='', outline=outlinecolor, dash=(dash1, dash2), style=tkinter.ARC,
                                              width=fillwidth, tag=shapeTag)
                    else:
                        drawCanvas.create_oval(x, y, x + 2 * oval_rx, y2, fill=fillcolor, outline=outlinecolor,
                                               dash=(dash1, dash2), width=fillwidth, tag=shapeTag)
                        drawCanvas.create_oval(x2 - 2 * oval_rx, y, x2, y2, fill=fillcolor, outline=outlinecolor,
                                               dash=(dash1, dash2), width=fillwidth, tag=shapeTag)
                        drawCanvas.create_rectangle(x + oval_rx, y, x2 - oval_rx, y2 + 1, fill=fillcolor,
                                                    outline=outlinecolor, dash=(dash1, dash2), width=fillwidth,
                                                    tag=shapeTag)
                        drawCanvas.create_line(x + oval_rx, y + half_width, x + oval_rx, y2 - half_width,
                                               fill=fillcolor, width=fillwidth, tag=shapeTag)
                        drawCanvas.create_line(x2 - oval_rx, y + half_width, x2 - oval_rx, y2 - half_width,
                                               fill=fillcolor, width=fillwidth, tag=shapeTag)
                else:
                    if fillcolor.find('#') < 0:
                        ...
                        extentAngle = 180
                        drawCanvas.create_arc(x, y, x + 2 * oval_rx, y2, start=startAngle, extent=extentAngle, fill='',
                                              outline=outlinecolor, style=tkinter.ARC, width=fillwidth, tag=shapeTag)
                        drawCanvas.create_line(x + oval_rx, y, x2 - oval_rx, y, fill=outlinecolor, width=fillwidth,
                                               tag=OutLineTag)
                        drawCanvas.create_line(x + oval_rx, y2, x2 - oval_rx, y2, fill=outlinecolor, width=fillwidth,
                                               tag=OutLineTag)
                        startAngle = 90
                        extentAngle = -180
                        drawCanvas.create_arc(x2 - 2 * oval_rx, y, x2, y2, start=startAngle, extent=extentAngle,
                                              fill='', outline=outlinecolor, style=tkinter.ARC, width=fillwidth,
                                              tag=shapeTag)
                    else:
                        drawCanvas.create_oval(x, y, x + 2 * oval_rx, y2, fill=fillcolor, outline=outlinecolor,
                                               width=fillwidth, tag=shapeTag)
                        drawCanvas.create_oval(x2 - 2 * oval_rx, y, x2, y2, fill=fillcolor, outline=outlinecolor,
                                               width=fillwidth, tag=shapeTag)
                        drawCanvas.create_rectangle(x + oval_rx + 1, y + half_width, x2 - oval_rx - 1,
                                                    y2 + 1 - half_width, fill=fillcolor, outline=outlinecolor, width=0,
                                                    tag=shapeTag)
                        drawCanvas.create_line(x + oval_rx, y, x2 - oval_rx, y, fill=outlinecolor, width=fillwidth,
                                               tag=OutLineTag)
                        drawCanvas.create_line(x + oval_rx, y2, x2 - oval_rx, y2, fill=outlinecolor, width=fillwidth,
            ...
            if len(text) > 0:
                drawCanvas.create_text(center_x, center_y, fill=textColor, text=text, font=textFont, anchor='center',
                                       tag=shapeTag + "_text")
            ...
            fillcolor = '#777777'
            drawCanvas.create_oval(x, y, x + SwitchHeight, y + SwitchHeight - 1, fill=fillcolor, outline=outlinecolor,
                                   width=0, tag=shapeTag)
            drawCanvas.create_oval(x + (SwitchWidth - SwitchHeight), y, x + SwitchWidth, y + SwitchHeight - 1,
                                   fill=fillcolor, outline=outlinecolor, width=0, tag=shapeTag)
            drawCanvas.create_rectangle(x + Switch_radius, y, x + (SwitchWidth - Switch_radius), y + SwitchHeight,
                                        fill=fillcolor, outline=outlinecolor, width=0, tag=shapeTag)
            drawCanvas.create_oval(x + 2, y + 2, x + (SwitchHeight - 3), y + (SwitchHeight - 3), fill=outlinecolor,
                                   width=0, tag=shapeTag)
            drawCanvas.create_text(x + (SwitchWidth - int(1.0 * SwitchHeight)), y + int(SwitchHeight / 2), text="Off",
                                   font=("Microsoft YaHei UI", int(SwitchHeight / 2)), anchor='center',
                                   fill=outlinecolor, width=0, tag=shapeTag)
        elif shapeType == 'listmenu':
            if dash1 > 0:
                drawCanvas.create_rectangle(x, y, x2, y2, fill='#FFFFFF', outline=outlinecolor, dash=(dash1, dash2),
                                            width=fillwidth, tag=shapeTag)
            else:
                drawCanvas.create_rectangle(x, y, x2, y2, fill='#FFFFFF', outline=outlinecolor, width=fillwidth,
                                            tag=shapeTag)
            MenuInfo = newImage
            ...
            for subMenu in SubMenus:
                ...
                drawCanvas.create_oval(SubMeshX, ListMenuTop, SubMeshX + SubMenuTitleHeight,
                                       ListMenuTop + SubMenuTitleHeight - 1, fill=fillcolor, outline=outlinecolor,
                                       width=0, tag=subMeshTag)
                drawCanvas.create_oval(x2 - SubMenuTitleHeight, ListMenuTop, x2, ListMenuTop + SubMenuTitleHeight - 1,
                                       fill=fillcolor, outline=outlinecolor, width=0, tag=subMeshTag)
                drawCanvas.create_rectangle(x + SubMenuTitleHeight_Half, ListMenuTop, x2 - SubMenuTitleHeight_Half,
                                            ListMenuTop + SubMenuTitleHeight, fill=fillcolor, outline=outlinecolor,
                                            width=0, tag=subMeshTag)
                centerY = ListMenuTop + int(SubMenuTitleHeight / 2)
                drawCanvas.create_text(centerX, centerY, text=titleText, anchor=tkinter.CENTER,
                                       font=('Arial', 14, 'bold'), fill=outlinecolor, tag=subMeshTag)
                ...
                if subMenu[3] == True:
                    for itemInfo in itemList:
                        ...
                        drawCanvas.create_oval(IconX - 5, centerY - 5, IconX + 5, centerY + 5, fill=fillcolor,
                                               outline=outlinecolor, width=0, tag=shapeTag)
                        drawCanvas.create_text(centerX, centerY, text=titleText, anchor=tkinter.CENTER,
                                               font=('Arial', 10, 'bold'), fill=outlinecolor, tag=shapeTag)
            ...
            if TableHeight > 0:
                ...
                for rowInfoLine in TableInfo['rows']:
                    ...
                    for rowInfo in rowInfoLine:
                        ...
                        if OutLineColor == '':
                            OutLineColor = '#000000'
                        drawCanvas.create_rectangle(x1, y1, x2, y2, fill=FillColor, outline=OutLineColor,
                                                    width=BorderWidth, tag='drawing_shape')
                        # 显示文字
                        if CellText != '':
                            if CellText.find("┇") >= 0:
                                drawCanvas.create_line(x1, y1, x2, y2, fill=OutLineColor, width=BorderWidth,
                                                       tag='drawing_shape')
                                ...
                                if FontIndex >= 0:
                                    ...
                                    fontName = FontInfo[0]
                                    fontSize = FontInfo[1]
                                    drawCanvas.create_text(cell_cx1, cell_cy1, fill=TextColor, text=Text1,
                                                           font=(fontName, fontSize), anchor=TextAnchor1,
                                                           tag='drawing_shape')
                                    drawCanvas.create_text(cell_cx2, cell_cy2, fill=TextColor, text=Text2,
                                                           font=(fontName, fontSize), anchor=TextAnchor2,
                                                           tag='drawing_shape')
                                else:
                                    drawCanvas.create_text(cell_cx1, cell_cy1, fill=TextColor, text=Text1,
                                                           anchor=TextAnchor1, tag='drawing_shape')
                                    drawCanvas.create_text(cell_cx2, cell_cy2, fill=TextColor, text=Text2,
                                                           anchor=TextAnchor2, tag='drawing_shape')
                            else:
                                cell_cx = int((x1 + x2) / 2)
                                cell_cy = int((y1 + y2) / 2)
                                ...
                                if FontIndex >= 0:
                                    ...
                                    fontName = FontInfo[0]
                                    fontSize = FontInfo[1]
                                    drawCanvas.create_text(cell_cx, cell_cy, fill=TextColor, text=CellText,
                                                           font=(fontName, fontSize), anchor=TextAnchor,
                                                           tag='drawing_shape')
                                else:
                                    drawCanvas.create_text(cell_cx, cell_cy, fill=TextColor, text=CellText,
                                                           anchor=TextAnchor, tag='drawing_shape')
                ...
                for mergeCell in TableInfo['merge']:
                    ...
                    if Right >= Left and Bottom >= Top:
                        drawCanvas.create_rectangle(Left, Top, Right, Bottom, fill=FillColor, outline=BorderColor,
                                                    width=BorderWidth, tag='drawing_shape')
                        # 显示文字
                        if CellText != '':
                            if CellText.find('┇') >= 0:
                                drawCanvas.create_line(Left, Top, Right, Bottom, fill=BorderColor, width=BorderWidth,
                                                       tag='drawing_shape')
                                TextSplitArray = CellText.split('┇')
                                Text1 = TextSplitArray[0]
                                Text2 = TextSplitArray[1]
...
def DrawLine(uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, width=1, dash=(0, 0), shapeTag=''):
    ...
    if shapeTag not in G_CanvasShapeDictionary[uiName][drawCanvasName]:
        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = ['line', x1, y1, x2, y2, Anchor, color, color,
                                                                     width, dash[0], dash[1]]
    ...
    G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [color, color, width, dash[0], dash[1], None, '', None,
                                                                 '']
    DoCanvasRecord(uiName, drawCanvasName, 'line', x1, y1, x2, y2, Anchor, color, color, width, dash1=dash[0],
                   dash2=dash[1], newImage=None, text='', textFont=None, textColor='', shapeTag=shapeTag)
    return shapeTag
...
def DrawArrow(uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, width=1, dash=(0, 0), shapeTag=''):
    ...
    if shapeTag not in G_CanvasShapeDictionary[uiName][drawCanvasName]:
        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = ['arrow', x1, y1, x2, y2, Anchor, color, color,
                                                                     width, dash[0], dash[1]]
    if drawCanvasName not in G_CanvasParamDictionary[uiName]:
        G_CanvasParamDictionary[uiName][drawCanvasName] = {}
    G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [color, color, width, dash[0], dash[1], None, '', None,
                                                                 '']
    DoCanvasRecord(uiName, drawCanvasName, 'arrow', x1, y1, x2, y2, Anchor, color, color, width, dash1=dash[0],
                   dash2=dash[1], newImage=None, text='', textFont=None, textColor='', shapeTag=shapeTag)
    return shapeTag


def DrawTriangle(uiName, drawCanvasName, direction, x1, y1, x2, y2, Anchor, color, outlinecolor='#FFFFFF',
                 outlinewidth=0, dash=(0, 0), shapeTag=''):
    PrintFunctionInfo(DrawTriangle,
                      [uiName, drawCanvasName, direction, x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth,
                       dash, shapeTag])
    # 在画布上画三角形
    drawCanvas = GetElement(uiName, drawCanvasName)
    if drawCanvas is None:
    ...
    G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = [TriangleType, x1, y1, x2, y2, Anchor, color,
                                                                 outlinecolor, outlinewidth, dash[0], dash[1]]
    if drawCanvasName not in G_CanvasParamDictionary[uiName]:
        G_CanvasParamDictionary[uiName][drawCanvasName] = {}
    G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [color, outlinecolor, outlinewidth, dash[0], dash[1],
                                                                 None, '', None, '']
    DoCanvasRecord(uiName, drawCanvasName, TriangleType, x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth,
                   dash1=dash[0], dash2=dash[1], newImage=None, text='', textFont=None, textColor='', shapeTag=shapeTag)
    return shapeTag


def DrawRectangle(uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor='#FFFFFF', outlinewidth=0,
                  dash=(0, 0), shapeTag=''):
    PrintFunctionInfo(DrawRectangle,
                      [uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth, dash,
                       shapeTag])
    # 在画布上画矩形
    drawCanvas = GetElement(uiName, drawCanvasName)
    if drawCanvas is None:
    ...
    if shapeTag not in G_CanvasShapeDictionary[uiName][drawCanvasName]:
        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = ['rect', x1, y1, x2, y2, Anchor, color,
                                                                     outlinecolor, outlinewidth, dash[0], dash[1]]
    if drawCanvasName not in G_CanvasParamDictionary[uiName]:
        G_CanvasParamDictionary[uiName][drawCanvasName] = {}
    G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [color, outlinecolor, outlinewidth, dash[0], dash[1],
                                                                 None, '', None, '']
    DoCanvasRecord(uiName, drawCanvasName, 'rect', x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth,
                   dash1=dash[0], dash2=dash[1], newImage=None, text='', textFont=None, textColor='', shapeTag=shapeTag)
    return shapeTag


def DrawRoundedRectangle(uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor='#FFFFFF', outlinewidth=0,
                         dash=(0, 0), roundRadius=10, shapeTag=''):
    PrintFunctionInfo(DrawRoundedRectangle,
                      [uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth, dash,
                       roundRadius, shapeTag])
    # 在画布上显示圆角矩形
    drawCanvas = GetElement(uiName, drawCanvasName)
    if drawCanvas is None:
        return
    ...
    if shapeTag not in G_CanvasShapeDictionary[uiName][drawCanvasName]:
        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = ['roundrect', x1, y1, x2, y2, Anchor, color,
                                                                     outlinecolor, outlinewidth, dash[0], dash[1]]
    if drawCanvasName not in G_CanvasParamDictionary[uiName]:
        G_CanvasParamDictionary[uiName][drawCanvasName] = {}
    G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [color, outlinecolor, outlinewidth, dash[0], dash[1],
                                                                 None, '', None, '']
    DoCanvasRecord(uiName, drawCanvasName, 'roundrect', x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth,
                   dash1=dash[0], dash2=dash[1], newImage=roundRadius, text='', textFont=None, textColor='',
                   shapeTag=shapeTag)
    return shapeTag


def DrawCircle(uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor='#FFFFFF', outlinewidth=0,
               dash=(0, 0), shapeTag=''):
    PrintFunctionInfo(DrawCircle,
                      [uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth, dash,
                       shapeTag])
    # 在画布上画圆
    drawCanvas = GetElement(uiName, drawCanvasName)
    ...
    if shapeTag not in G_CanvasShapeDictionary[uiName][drawCanvasName]:
        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = ['circle', x1, y1, x2, y2, Anchor, color,
                                                                     outlinecolor, outlinewidth, dash[0], dash[1]]
    if drawCanvasName not in G_CanvasParamDictionary[uiName]:
        G_CanvasParamDictionary[uiName][drawCanvasName] = {}
    G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [color, outlinecolor, outlinewidth, dash[0], dash[1],
                                                                 None, '', None, '']
    DoCanvasRecord(uiName, drawCanvasName, 'circle', x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth,
                   dash1=dash[0], dash2=dash[1], newImage=None, text='', textFont=None, textColor='', shapeTag=shapeTag)
    return shapeTag


def DrawDiamond(uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor='#FFFFFF', outlinewidth=0,
                dash=(0, 0), shapeTag=''):
    PrintFunctionInfo(DrawDiamond,
                      [uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth, dash,
                       shapeTag])
    # 在画布上画菱形
    drawCanvas = GetElement(uiName, drawCanvasName)
    ...
    if shapeTag not in G_CanvasShapeDictionary[uiName][drawCanvasName]:
        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = ['diamond', x1, y1, x2, y2, Anchor, color,
                                                                     outlinecolor, outlinewidth, dash[0], dash[1]]
    if drawCanvasName not in G_CanvasParamDictionary[uiName]:
        G_CanvasParamDictionary[uiName][drawCanvasName] = {}
    G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [color, outlinecolor, outlinewidth, dash[0], dash[1],
                                                                 None, '', None, '']
    DoCanvasRecord(uiName, drawCanvasName, 'diamond', x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth,
                   dash1=dash[0], dash2=dash[1], newImage=None, text='', textFont=None, textColor='', shapeTag=shapeTag)
    return shapeTag


def DrawCylinder(uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor='#FFFFFF', outlinewidth=0,
                 dash=(0, 0), shapeTag=''):
    PrintFunctionInfo(DrawCylinder,
                      [uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth, dash,
                       shapeTag])
    # 在画布上画圆柱
    drawCanvas = GetElement(uiName, drawCanvasName)
    ...
    if shapeTag not in G_CanvasShapeDictionary[uiName][drawCanvasName]:
        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = ['cylinder', x1, y1, x2, y2, Anchor, color,
                                                                     outlinecolor, outlinewidth, dash[0], dash[1]]
    if drawCanvasName not in G_CanvasParamDictionary[uiName]:
        G_CanvasParamDictionary[uiName][drawCanvasName] = {}
    G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [color, outlinecolor, outlinewidth, dash[0], dash[1],
                                                                 None, '', None, '']
    DoCanvasRecord(uiName, drawCanvasName, 'cylinder', x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth,
                   dash1=dash[0], dash2=dash[1], newImage=None, text='', textFont=None, textColor='', shapeTag=shapeTag)
    return shapeTag


def DrawStar(uiName, drawCanvasName, x1, y1, x2, y2, Anchor, color, outlinecolor='#FFFFFF', outlinewidth=0, dash=(0, 0),
             shapeTag=''):
    PrintFunctionInfo(DrawStar,
                      [uiName, drawCanvasName, x1, y1, x2, y2, color, outlinecolor, outlinewidth, dash, shapeTag])
    # 在画布上画星星
    drawCanvas = GetElement(uiName, drawCanvasName)
    if drawCanvas is None:
        return
    ...
    if shapeTag not in G_CanvasShapeDictionary[uiName][drawCanvasName]:
        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = ['star', x1, y1, x2, y2, Anchor, color,
                                                                     outlinecolor, outlinewidth, dash[0], dash[1]]
    if drawCanvasName not in G_CanvasParamDictionary[uiName]:
        G_CanvasParamDictionary[uiName][drawCanvasName] = {}
    G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [color, outlinecolor, outlinewidth, dash[0], dash[1],
                                                                 None, '', None, '']
    DoCanvasRecord(uiName, drawCanvasName, 'star', x1, y1, x2, y2, Anchor, color, outlinecolor, outlinewidth,
                   dash1=dash[0], dash2=dash[1], newImage=None, text='', textFont=None, textColor='', shapeTag=shapeTag)
    return shapeTag
...
def DrawImage(uiName, drawCanvasName, x1, y1, x2, y2, Anchor, imagefile, shapeTag=''):
    ...
    if newImage:
        if shapeTag not in G_CanvasShapeDictionary[uiName][drawCanvasName]:
            G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = ['image', x1, y1, x2, y2, Anchor, newImage,
                                                                         imagefile]
        if drawCanvasName not in G_CanvasParamDictionary[uiName]:
            G_CanvasParamDictionary[uiName][drawCanvasName] = {}
        G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = ['#FFFFFF', '#FFFFFF', 0, 0, 0, newImage, '', None,
                                                                     '#FFFFFF']
        DoCanvasRecord(uiName, drawCanvasName, 'image', x1, y1, x2, y2, Anchor, '#FFFFFF', '#FFFFFF', 0, dash1=0,
                       dash2=0, newImage=newImage, text='', textFont=None, textColor='', shapeTag=shapeTag)
        if hasGIFAnimation == True:
            drawCanvas.after(100, lambda: updateGIFFrame(uiName, drawCanvasName))


def DrawButton(uiName, drawCanvasName, x1, y1, x2, y2, Anchor, text='', textcolor='#000000', textFont=None,
               fillcolor='#FFFFFF', outlinecolor='#FFFFFF', outlinewidth=0, dash=(0, 0), shapeTag=''):
    PrintFunctionInfo(DrawButton,
                      [uiName, drawCanvasName, x1, y1, x2, y2, text, textcolor, textFont, fillcolor, outlinecolor,
                       outlinewidth, dash, shapeTag])
    # 在画布上显示圆角按钮
    drawCanvas = GetElement(uiName, drawCanvasName)
    if drawCanvas is None:
        return
    ...
    if shapeTag not in G_CanvasShapeDictionary[uiName][drawCanvasName]:
        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = ['button', x1, y1, x2, y2, Anchor, text, textcolor,
                                                                     textFont, fillcolor, outlinecolor, outlinewidth,
                                                                     dash[0], dash[1], None]
    if drawCanvasName not in G_CanvasParamDictionary[uiName]:
        G_CanvasParamDictionary[uiName][drawCanvasName] = {}
    G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [fillcolor, outlinecolor, outlinewidth, dash[0],
                                                                 dash[1], None, text, textFont, textcolor]
    if dash1 > 0:
        drawCanvas.create_oval(x1, y1, x1 + 2 * oval_rx, y2, fill=fillcolor, outline=outlinecolor, dash=(dash1, dash2),
                               width=outlinewidth, tag=shapeTag)
        drawCanvas.create_oval(x2 - 2 * oval_rx, y1, x2, y2, fill=fillcolor, outline=outlinecolor, dash=(dash1, dash2),
                               width=outlinewidth, tag=shapeTag)
        drawCanvas.create_rectangle(x1 + oval_rx, y1, x2 - oval_rx, y2, fill=fillcolor, outline=outlinecolor,
                                    dash=(dash1, dash2), width=outlinewidth, tag=shapeTag)
        drawCanvas.create_line(x1 + oval_rx, y1 + half_width, x1 + oval_rx, y2 - half_width, fill=fillcolor,
                               width=outlinewidth, tag=OutLineTag)
        drawCanvas.create_line(x2 - oval_rx, y1 + half_width, x2 - oval_rx, y2 - half_width, fill=fillcolor,
                               width=outlinewidth, tag=OutLineTag)
    else:
        drawCanvas.create_oval(x1, y1, x1 + 2 * oval_rx, y2, fill=fillcolor, outline=outlinecolor, width=outlinewidth,
                               tag=shapeTag)
        drawCanvas.create_oval(x2 - 2 * oval_rx, y1, x2, y2, fill=fillcolor, outline=outlinecolor, width=outlinewidth,
                               tag=shapeTag)
        drawCanvas.create_rectangle(x1 + oval_rx, y1, x2 - oval_rx, y2, fill=fillcolor, outline=outlinecolor,
                                    width=outlinewidth, tag=shapeTag)
        drawCanvas.create_line(x1 + oval_rx, y1 + half_width, x1 + oval_rx, y2 - half_width, fill=fillcolor,
...
def EraserCanvas(uiName, drawCanvasName, x1, y1, x2, y2):
    ...
    DoCanvasRecord(uiName, drawCanvasName, 'eraser', x1, y1, x2, y2, 'nw', bgcolor, bgcolor, 0, dash1=0, dash2=0,
                   newImage=None, text='', textFont=None, textColor='', shapeTag='')
...
def SetCanvasGridBG(uiName, drawCanvasName, bgcolor='#888888', tile_width=20, tile_height=20):
    ...
    DoCanvasRecord(uiName, drawCanvasName, 'grid', 0, 0, canvasWidth, canvasHeight, 'nw', bgcolor, bgcolor, 0,
                   dash1=tile_width, dash2=tile_height, newImage=None, text='', textFont=None, textColor='',
                   shapeTag='grid_bg')
...
def Shape_MouseEvent(event, uiName, canvasName, shapeTag, eventName):
    ...
    if eventName == 'MouseLeave':
        ...
        if G_CanvasShapeDictionary[uiName][canvasName][shapeTag][0] == 'button':
            borderwidth = 1 + G_CanvasShapeDictionary[uiName][canvasName][shapeTag][11]
                    ...
                    if type(imageInfo[1]) == type([]):
                        ...
                        if elementName.find('Text_') >= 0:
                            if GifData[3]:
                                ...
                                GifData[3] = Control.image_create(tkinter.END, image=FrameSequ[FrameIndex])
                        elif elementName.find('Label_') >= 0 or elementName.find('Button_') >= 0 or elementName.find(
                                'RadioButton_') >= 0 or elementName.find('CheckButton_') >= 0:
                ...
                if drawCanvas != None:
                    ...
                    if ShapeType == 'image':
                        ...
                        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = [ShapeType, x1, y1, x2, y2,
                                                                                     AnchorText, newImage, imagefile]
                        G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [fill, outline, width, dashx, dashy,
                                                                                     newImage, newtext, textFont,
                                                                                     textColor]
                        DoCanvasRecord(uiName, drawCanvasName, ShapeType, x1, y1, x2, y2, AnchorText, fill, outline,
                                       width, dashx, dashy, newImage, newtext, textFont, textColor, shapeTag)
                    elif ShapeType == 'text':
                        if splitArray[1].find('.') > 0:
                            x1 = float(splitArray[1])
                        else:
                        ...
                        textColor = ''
                        textFont = tkinter.font.Font(family=familytext, size=sizetext, weight=weighttext,
                                                     slant=slanttext, underline=underlinetext,
                                                     overstrike=overstriketext)
                        shapeTag = splitArray[-2]
                        AnchorText = splitArray[-1]
                        if AnchorText.find("|") >= 0:
                            AnchorText = AnchorText.split("|")[1]
                        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = [ShapeType, x1, y1, x2, y2,
                                                                                     AnchorText, newtext, textFont,
                                                                                     fill]
                        # 字体
                        fontFind = False
                        for fontInfo in G_CanvasFontDictionary[uiName][drawCanvasName]:
                            if fontInfo[1] == familytext and fontInfo[2] == sizetext and fontInfo[3] == weighttext and \
                                    fontInfo[4] == slanttext and fontInfo[5] == underlinetext and fontInfo[
                                6] == overstriketext:
                                fontFind = True
                                continue
                        if fontFind == False:
                            G_CanvasFontDictionary[uiName][drawCanvasName].append(
                                [textFont, familytext, sizetext, weighttext, slanttext, underlinetext, overstriketext])
                        G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [fill, outline, width, dashx, dashy,
                                                                                     newImage, newtext, textFont,
                                                                                     textColor]
                        DoCanvasRecord(uiName, drawCanvasName, ShapeType, x1, y1, x2, y2, AnchorText, fill, outline,
                        ...
                        if len(familytext) > 0:
                            ...
                            fontFind = False
                            for fontInfo in G_CanvasFontDictionary[uiName][drawCanvasName]:
                                if fontInfo[1] == familytext and fontInfo[2] == sizetext and fontInfo[
                                    3] == weighttext and fontInfo[4] == slanttext and fontInfo[5] == underlinetext and \
                                        fontInfo[6] == overstriketext:
                                    fontFind = True
                                    continue
                            if fontFind == False:
                                G_CanvasFontDictionary[uiName][drawCanvasName].append(
                                    [textFont, familytext, sizetext, weighttext, slanttext, underlinetext,
                                     overstriketext])
                        ...
                        if AnchorText.find("|") >= 0:
                            AnchorText = AnchorText.split("|")[1]
                        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = [ShapeType, x1, y1, x2, y2,
                                                                                     AnchorText, newtext, textColor,
                                                                                     textFont, fill, outline, width,
                                                                                     dashx, dashy, newImage]
                        G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [fill, outline, width, dashx, dashy,
                                                                                     newImage, newtext, textFont,
                                                                                     textColor]
                        DoCanvasRecord(uiName, drawCanvasName, ShapeType, x1, y1, x2, y2, AnchorText, fill, outline,
                        ...
                        if len(splitArray) > 11:
                            ...
                            if AnchorText.find("|") >= 0:
                                AnchorText = AnchorText.split("|")[1]
                            G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = [ShapeType, x1, y1, x2, y2,
                                                                                         AnchorText, fill, outline,
                                                                                         width, dashx, dashy]
                            G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [fill, outline, width, dashx,
                                                                                         dashy, newImage, newtext,
                                                                                         textFont, textColor]
                            DoCanvasRecord(uiName, drawCanvasName, ShapeType, x1, y1, x2, y2, AnchorText, fill, outline,
                                           width, dashx, dashy, newImage, newtext, textFont, textColor, shapeTag)
                    ...
                    #                         DoCanvasRecord(uiName,drawCanvasName,ShapeType,x1,y1,x2,y2,AnchorText,fill,outline,width,dashx,dashy,newImage,newtext,textFont,textColor,shapeTag)
                        ...
                        table_dict = {}
                        table_dict['font'] = []
                        table_dict['font'].append(['Microsoft YaHei UI', '12'])
                        table_dict['style'] = []
                        ...
                        textColor = ''
                        G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag] = [ShapeType, x1, y1, x2, y2,
                                                                                     AnchorText, fill, outline, width,
                                                                                     dashx, dashy, table_dict]
                        G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag] = [fill, outline, width, dashx, dashy,
                                                                                     newImage, newtext, textFont,
                                                                                     textColor]
                        DoCanvasRecord(uiName, drawCanvasName, ShapeType, x1, y1, x2, y2, AnchorText, fill, outline,
                ...
                for shapeTag in G_CanvasEventDictionary[uiName][drawCanvasName].keys():
                    for EventName in G_CanvasEventDictionary[uiName][drawCanvasName][shapeTag]:
                        ...
                        if EventName == "MouseEnter":
                            drawCanvas.tag_bind(shapeTag, "<Any-Enter>",
                                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                      canvasName=drawCanvasName, shapeTag=shapeTag,
                                                                      eventName="MouseEnter"))
                            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName] and \
                                    G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                                TextTag = shapeTag + "_text"
                                drawCanvas.tag_bind(TextTag, "<Any-Enter>",
                                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                          canvasName=drawCanvasName, shapeTag=shapeTag,
                                                                          eventName="MouseEnter"))
                        elif EventName == "MouseLeave":
                            drawCanvas.tag_bind(shapeTag, "<Any-Leave>",
                                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                      canvasName=drawCanvasName, shapeTag=shapeTag,
                            ...
                            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName] and \
                                    G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                                TextTag = shapeTag + "_text"
                                drawCanvas.tag_bind(TextTag, "<Any-Leave>",
                                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                          canvasName=drawCanvasName, shapeTag=shapeTag,
                                                                          eventName="MouseLeave"))
                        elif EventName == "ButtonDown":
                            drawCanvas.tag_bind(shapeTag, "<Button-1>",
                                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                      canvasName=drawCanvasName, shapeTag=shapeTag,
                                                                      eventName="ButtonDown"))
                            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName] and \
                                    G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                                TextTag = shapeTag + "_text"
                                drawCanvas.tag_bind(TextTag, "<Button-1>",
                                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                          canvasName=drawCanvasName, shapeTag=shapeTag,
                                                                          eventName="ButtonDown"))
                        elif EventName == "ButtonMotion":
                            drawCanvas.tag_bind(shapeTag, "<B1-Motion>",
                                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                      canvasName=drawCanvasName, shapeTag=shapeTag,
                                                                      eventName="ButtonMotion"))
                            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName] and \
                                    G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                                TextTag = shapeTag + "_text"
                                drawCanvas.tag_bind(TextTag, "<B1-Motion>",
                                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                          canvasName=drawCanvasName, shapeTag=shapeTag,
                                                                          eventName="ButtonMotion"))
                        elif EventName == "ButtonUp":
                            drawCanvas.tag_bind(shapeTag, "<ButtonRelease-1>",
                                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                      canvasName=drawCanvasName, shapeTag=shapeTag,
                                                                      eventName="ButtonUp"))
                            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName] and \
                                    G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                                TextTag = shapeTag + "_text"
                                drawCanvas.tag_bind(TextTag, "<ButtonRelease-1>",
                                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                          canvasName=drawCanvasName, shapeTag=shapeTag,
                                                                          eventName="ButtonUp"))
                        elif EventName == "DoubleClick":
                            drawCanvas.tag_bind(shapeTag, "<Double-1>",
                                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                      canvasName=drawCanvasName, shapeTag=shapeTag,
                            ...
                            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName] and \
                                    G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                                TextTag = shapeTag + "_text"
                                drawCanvas.tag_bind(TextTag, "<Double-1>",
                                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName,
                                                                          canvasName=drawCanvasName, shapeTag=shapeTag,
                                                                          eventName="DoubleClick"))
    ...
    if uiName in G_CanvasShapeDictionary:
        if drawCanvasName in G_CanvasShapeDictionary[uiName]:
            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
                ...
                textcolor = G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][8]
            ...
            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
                ...
                for mergeInfo in TableInfo['merge']:
                    ...
                    if checkPtInRect(col, row, MergeBeginCow, MergeEndCow, MergeBeginRow, MergeEndRow) == True:
                        ...
                        Params[5] = TableInfo
                        DoCanvasRecord(uiName, drawCanvasName, 'table', x1, y1, x2, y2, RefPointText, Params[0],
        ...
        if drawCanvasName in G_CanvasShapeDictionary[uiName]:
            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
                ...
                for FontInfo in TableInfo['font']:
                    if FontInfo[0] == familytext and FontInfo[1] == sizetext and FontInfo[2] == weighttext and FontInfo[
                        3] == slanttext and FontInfo[4] == underlinetext and FontInfo[5] == overstriketext:
                        CellTextFontIndex = FontIndex
                        break
                    FontIndex = FontIndex + 1
                if CellTextFontIndex < 0:
                    CellTextFontIndex = len(TableInfo['font'])
                    TableInfo['font'].append(
                        [familytext, sizetext, weighttext, slanttext, underlinetext, overstriketext])
                for mergeInfo in TableInfo['merge']:
                    MergeBeginRow = mergeInfo[0][0]
                    MergeBeginCow = mergeInfo[0][1]
                    MergeEndRow = mergeInfo[1][0]
    ...
    if uiName in G_CanvasShapeDictionary:
        if drawCanvasName in G_CanvasShapeDictionary[uiName]:
            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
                ...
                Params = G_CanvasParamDictionary[uiName][drawCanvasName][shapeTag]
                Params[5] = TableInfo
                DoCanvasRecord(uiName, drawCanvasName, 'table', x1, y1, x2, y2, RefPointText, Params[0], Params[1],
                               Params[2], Params[3], Params[4], Params[5], Params[6], Params[7], Params[8], shapeTag)
            ...
            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
                ...
                for mergeInfo in TableInfo['merge']:
                    ...
                    for rowInfoLine in TableInfo['rows']:
                        CowIndex = 0
                        for rowInfo in rowInfoLine:
                            if checkPtInRect(CowIndex, RowIndex, MergeBeginCow, MergeEndCow, MergeBeginRow,
                                             MergeEndRow) == True:
                                rowInfo[1] = 1
                        CowIndex = CowIndex + 1
                    RowIndex = RowIndex + 1
...
def BindShapeEvent_SetShapeRect(uiName, drawCanvasName, shapeTag, bindEvent, targetShapeTag, x, y, w, h):
    PrintFunctionInfo(BindShapeEvent_SetShapeRect,
                      [uiName, drawCanvasName, shapeTag, bindEvent, targetShapeTag, x, y, w, h])
    # 绑定事件-设置图形位置与大小
    if uiName in G_CanvasShapeDictionary:
        if drawCanvasName in G_CanvasShapeDictionary[uiName]:
            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
...
def BindShapeEvent_SetOutlineColor(uiName, drawCanvasName, shapeTag, bindEvent, targetShapeTag, color):
    PrintFunctionInfo(BindShapeEvent_SetOutlineColor,
                      [uiName, drawCanvasName, shapeTag, bindEvent, targetShapeTag, color])
    # 绑定事件-设置图形边框颜色
    if uiName in G_CanvasShapeDictionary:
        if drawCanvasName in G_CanvasShapeDictionary[uiName]:
            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
...
def BindShapeEvent_ChangeImage(uiName, drawCanvasName, shapeTag, bindEvent, targetShapeTag, ImageFile):
    PrintFunctionInfo(BindShapeEvent_ChangeImage,
                      [uiName, drawCanvasName, shapeTag, bindEvent, targetShapeTag, ImageFile])
    # 绑定事件-更换图形图片
    if uiName in G_CanvasShapeDictionary:
        if drawCanvasName in G_CanvasShapeDictionary[uiName]:
            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
...
def BindShapeEvent_ChangeText(uiName, drawCanvasName, shapeTag, bindEvent, targetShapeTag, Text, TextColor):
    PrintFunctionInfo(BindShapeEvent_ChangeText,
                      [uiName, drawCanvasName, shapeTag, bindEvent, targetShapeTag, Text, TextColor])
    # 绑定事件-设置图形文字
    if uiName in G_CanvasShapeDictionary:
        if drawCanvasName in G_CanvasShapeDictionary[uiName]:
            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
...
def BindShapeEvent_CallFunction(uiName, drawCanvasName, shapeTag, bindEvent, targetShapeTag, callBackFuncton,
                                param=None):
    PrintFunctionInfo(BindShapeEvent_CallFunction,
                      [uiName, drawCanvasName, shapeTag, bindEvent, targetShapeTag, callBackFuncton, param])
    # 绑定事件-调用函数
    if uiName in G_CanvasShapeDictionary:
        if drawCanvasName in G_CanvasShapeDictionary[uiName]:
            if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
...
def BindShapeMouseEvent(uiName, drawCanvasName, shapeTag, bindEvent, actionInfo):
    ...
    if uiName in G_CanvasShapeDictionary:
        ...
        if bindEvent == "MouseEnter":
            drawCanvas.tag_bind(shapeTag, "<Any-Enter>",
                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
                                                      shapeTag=shapeTag, eventName="MouseEnter"))
            if G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                TextTag = shapeTag + "_text"
                drawCanvas.tag_bind(TextTag, "<Any-Enter>",
                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
            ...
            drawCanvas.tag_bind(shapeTag, "<Any-Leave>",
                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
                                                      shapeTag=shapeTag, eventName="MouseLeave"))
            if G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                TextTag = shapeTag + "_text"
                drawCanvas.tag_bind(TextTag, "<Any-Leave>",
                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
            ...
            drawCanvas.tag_bind(shapeTag, "<Button-1>",
                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
                                                      shapeTag=shapeTag, eventName="ButtonDown"))
            if G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                TextTag = shapeTag + "_text"
                drawCanvas.tag_bind(TextTag, "<Button-1>",
                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
            ...
            drawCanvas.tag_bind(shapeTag, "<B1-Motion>",
                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
                                                      shapeTag=shapeTag, eventName="ButtonMotion"))
            if G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                TextTag = shapeTag + "_text"
                drawCanvas.tag_bind(TextTag, "<B1-Motion>",
                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
            ...
            drawCanvas.tag_bind(shapeTag, "<ButtonRelease-1>",
                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
                                                      shapeTag=shapeTag, eventName="ButtonUp"))
            if G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                TextTag = shapeTag + "_text"
                drawCanvas.tag_bind(TextTag, "<ButtonRelease-1>",
                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
            ...
            drawCanvas.tag_bind(shapeTag, "<Double-1>",
                                EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
                                                      shapeTag=shapeTag, eventName="DoubleClick"))
            if G_CanvasShapeDictionary[uiName][drawCanvasName][shapeTag][0] == 'button':
                TextTag = shapeTag + "_text"
                drawCanvas.tag_bind(TextTag, "<Double-1>",
                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
    ...
    if drawCanvasName in G_CanvasShapeDictionary[uiName]:
        if shapeTag in G_CanvasShapeDictionary[uiName][drawCanvasName]:
            ...
            if actionInfo[2] == False:
                fillcolor = '#777777'
                drawCanvas.create_oval(x1, y1, x1 + SwitchHeight, y1 + SwitchHeight - 1, fill=fillcolor,
                                       outline=outlinecolor, width=0, tag=shapeTag)
                drawCanvas.create_oval(x1 + (SwitchWidth - SwitchHeight), y1, x1 + SwitchWidth, y1 + SwitchHeight - 1,
                                       fill=fillcolor, outline=outlinecolor, width=0, tag=shapeTag)
                drawCanvas.create_rectangle(x1 + Switch_radius, y1, x1 + (SwitchWidth - Switch_radius),
                                            y1 + SwitchHeight, fill=fillcolor, outline=outlinecolor, width=0,
                                            tag=shapeTag)
                drawCanvas.create_oval(x1 + 2, y1 + 2, x1 + (SwitchHeight - 3), y1 + (SwitchHeight - 3),
                                       fill=outlinecolor, width=0, tag=shapeTag)
                drawCanvas.create_text(x1 + (SwitchWidth - int(1.0 * SwitchHeight)), y1 + int(SwitchHeight / 2),
                                       text="Off", font=("Microsoft YaHei UI", int(SwitchHeight / 2)), anchor='center',
                                       fill=outlinecolor, width=0, tag=shapeTag)
                actionInfo[2] = True
            else:
                drawCanvas.create_oval(x1, y1, x1 + SwitchHeight, y1 + SwitchHeight - 1, fill=fillcolor,
                ...
                drawCanvas.create_rectangle(x1 + Switch_radius, y1, x1 + (SwitchWidth - Switch_radius),
                                            y1 + SwitchHeight, fill=fillcolor, outline=outlinecolor, width=0,
                                            tag=shapeTag)
                drawCanvas.create_oval(x1 + (SwitchWidth - SwitchHeight) - 2, y1 + 2, x1 + SwitchWidth - 3,
                                       y1 + (SwitchHeight - 3), fill=outlinecolor, width=0, tag=shapeTag)
                drawCanvas.create_text(x1 + int(0.8 * SwitchHeight), y1 + int(SwitchHeight / 2), text="On",
                                       font=("Microsoft YaHei UI", int(SwitchHeight / 2)), anchor='center',
                                       fill=outlinecolor, width=0, tag=shapeTag)
                actionInfo[2] = False
            ...
            for subMenu in SubMenus:
                ...
                if shapeTag == subMenuTag:
                    if subMenu[3] == True:
                        ...
                        subMenu[3] = True
            DoCanvasRecord(uiName, drawCanvasName, "listmenu", x1, y1, x2, y2, fillcolor, outlinecolor, fillwidth,
                           dash1=0, dash2=0, newImage=MenuInfo, text='', textFont=None, textColor='',
                           shapeTag=listmenuName)
            for subMenu in SubMenus:
                titleText = subMenu[0]
                subMenuTag = listmenuName + "_" + titleText
                drawCanvas.tag_bind(subMenuTag, "<Button-1>",
                                    EventFunction_Adaptor(Shape_MouseEvent, uiName=uiName, canvasName=drawCanvasName,
                                                          shapeTag=subMenuTag, eventName="ButtonDown"))
                ...
                for shapeTag in G_CanvasShapeDictionary[uiName][canvasName]:
                    ...
                    if ShapeType == 'image':
                        ...
                        if newImage == None:
                            ...
                            DoCanvasRecord(uiName, canvasName, ShapeType, x1, y1, x2, y2, AnchorText, Params[0],
                                           Params[1], Params[2], Params[3], Params[4], Params[5], Params[6], Params[7],
                                           Params[8], shapeTag)
                    elif ShapeType == 'text':
                        x1 = G_CanvasShapeDictionary[uiName][canvasName][shapeTag][1]
                        ...
                        drawCanvas.delete(shapeTag)
                        Params = G_CanvasParamDictionary[uiName][canvasName][shapeTag]
                        DoCanvasRecord(uiName, canvasName, ShapeType, x1, y1, x2, y2, AnchorText, Params[0], Params[1],
                                       Params[2], Params[3], Params[4], Params[5], Params[6], Params[7], Params[8],
                                       shapeTag)
                    elif ShapeType == 'button':
                        x1 = G_CanvasShapeDictionary[uiName][canvasName][shapeTag][1]
    ...
    if uiName in G_CanvasSizeDictionary:
        for canvasName in G_CanvasSizeDictionary[uiName]:
            ...
            if canvasName == "Form_1":
                ...
                drawCanvas_width = root.winfo_width()
                drawCanvas_height = root.winfo_height()
            if ForceReDraw == True or G_CanvasSizeDictionary[uiName][canvasName][0] != drawCanvas_width or \
                    G_CanvasSizeDictionary[uiName][canvasName][1] != drawCanvas_height:
                ReDraw = True
    ...
    for groupid in PyMeStyleRadioGroup.keys():
        for RadioInfo in PyMeStyleRadioGroup[groupid]:
            if RadioInfo["radio"] is event.widget:
                ...
                RadioInfo["icon"].place(x=radio_x + 4, y=radio_y + radio_height // 2 - radio_circle_size // 2,
                                        width=radio_circle_size, height=radio_circle_size)
        ...
        if GroupName.find("Group_") == 0:
            ...
            if Control:
                ...
                Control.bind("<ButtonRelease-1>",
                             lambda event: OnRadioButtonClick2(uiName, elementName, GroupID, radiovalue))
                ...
                small_radio_icon.place(x=radio_x + 4, y=radio_y + radio_height // 2 - radio_circle_size // 2,
                                       width=radio_circle_size, height=radio_circle_size)
                small_radio_icon.bind("<Button-1>",
                                      lambda event: OnRadioButtonClick1(uiName, elementName, GroupID, radiovalue))
                small_radio_icon.bind("<ButtonRelease-1>",
                                      lambda event: OnRadioButtonClick2(uiName, elementName, GroupID, radiovalue, True))
                if GroupID not in PyMeStyleRadioGroup:
                    PyMeStyleRadioGroup[GroupID] = []
                NewRadioInfo = {"name": elementName, "radio": Control, "var": radiovalue, "icon": small_radio_icon,
                                "border_color": SelectedFGColor, "image_no": image_no, "image_yes": image_yes,
                                "image_click": image_click, "tkimage_no": tkimage_no, "tkimage_yes": tkimage_yes,
                                "tkimage_click": tkimage_click}
    ...
    for uiName in PyMeStyleCheckButton.keys():
        for CheckInfo in PyMeStyleCheckButton[uiName]:
            if CheckInfo["checkbutton"] is event.widget:
                ...
                if radio_circle_size < 26:
                    radio_circle_size = 26
                CheckInfo["icon"].place(x=radio_x + 4, y=radio_y + radio_height // 2 - radio_circle_size // 2,
                                        width=radio_circle_size, height=radio_circle_size)


def SetCheckButtonPyMeStyle(uiName, elementName, checkbutton_value, fgColor, SelectedFGColor, visible=True):
    PrintFunctionInfo(SetCheckButtonPyMeStyle,
                      [uiName, elementName, checkbutton_value, fgColor, SelectedFGColor, visible])
    global PyMeStyleCheckButton
    Control = GetElement(uiName, elementName)
    if Control:
        text = Control.cget("text")
        ...
        NewCheckButtonInfo = {"checkbutton": Control, "icon": small_radio_icon, "border_color": SelectedFGColor,
                              "image_no": image_no, "image_yes": image_yes, "image_click": image_click,
                              "tkimage_no": tkimage_no, "tkimage_yes": tkimage_yes, "tkimage_click": tkimage_click}
...
def ListView_CtrlCCopy_CallBack(event):
    Control = event.widget
    items = Control.selection()
    if items:
        item_text = ''
        for i in range(len(items)):
            item = str(items[i])
            values = Control.item(item, "values")
            if i == 0:
                item_text = str(values)
            else:
                item_text = item_text + str(values)
        if item_text != '':
            import pyperclip
...
class WindowDraggable():
    ...
    def MoveDragPos(self, event):
        state = self.widget.state()
        if state == "normal":
            if self.widget == event.widget or event.widget.winfo_class() == "Canvas" or event.widget.winfo_class() == "Message" or event.widget.winfo_class() == "Label" or event.widget.winfo_class() == "Frame" or event.widget.winfo_class() == "Labelframe":
                formx = self.widget.winfo_x()
                formy = self.widget.winfo_y()
...
class ToolTip(object):
    def __init__(self, widget, bgColor='#CCCCCC', fgColor='#000000', inTop=False):
        ...
        self.x = 0
        self.y = 0
        self.font = tkinter.font.Font(family="Microsoft YaHei UI", size=12, weight='normal', slant='roman', underline=0,
                                      overstrike=0)
    ...
    def showtip(self, text, x, y):
        ...
        self.tipwindow.configure(background=self.bgColor)
        if type(text) == type([]):
            self.Text = tkinter.Text(self.tipwindow, width=maxwidth, background=self.bgColor, fg=self.fgColor,
                                     relief=tkinter.FLAT, borderwidth=2, font=self.font)
            ...
            self.label = tkinter.Message(self.tipwindow, text=self.text, anchor=tkinter.W, width=maxwidth,
                                         background=self.bgColor, fg=self.fgColor, relief=tkinter.FLAT, borderwidth=2,
                                         font=self.font)
...
def OpenFile(title="Open Python File", filetypes=[('Python File', '*.py'), ('All files', '*')], initDir='',
             multi=False):
    PrintFunctionInfo(OpenFile, [title, filetypes, initDir, multi])
    # 调用打开文件框
    import tkinter.filedialog
    import inspect
    parent = None
    ...
    if multi == False:
        ...
        openPath = tkinter.filedialog.askopenfilenames(initialdir=initDir, title=title, filetypes=filetypes,
                                                       parent=parent)
        return openPath
...
def SaveFile(title="Save Python File", filetypes=[('Python File', '*.py'), ('All files', '*')], initDir='',
             initialFile='', defaultextension='py'):
    ...
    if "uiName" in calling_frame.f_locals:
        uiName = calling_frame.f_locals["uiName"]
        parent = GetElement(uiName, "Form_1")
    savePath = tkinter.filedialog.asksaveasfilename(initialdir=initDir, initialfile=initialFile, title=title,
    ...
    while True:
        ...
        if text.find('style.configure(') >= 0:
            ...
            if fontindex_begin >= 0:
                ...
                StyleArray[stylename]['font'] = tkinter.font.Font(family=fontsplitarray[0].replace('"', '').strip(),
                                                                  size=int(fontsplitarray[1].replace('"', '').strip()),
                                                                  weight=fontsplitarray[2].replace('"', '').strip())
...
class DownLoadFileProgressDialog:
    def __init__(self, uiName, showDialog=True, title='"+Language.G_Language[3240]+"', bgColor='#f8f9fa',
                 fgColor='#000000'):
    ...
    def downloadFileFromURL(self, url, saveToDir=None, ReDownLoadIfExist=True, autoExtractZip=False,
                            progressCallBack=None, finishCallBack=None, errorCallBack=None):
    ...
    def downloadFilesFromURLList(self, urllist, saveToDir, ReDownLoadIfExist=True, progressCallBack=None,
                                 finishCallBack=None, errorCallBack=None):
    ...
    def ShowDownLoadProgressDialog(self):
        if self.showDialog == True:
            self.TitleFont = tkinter.font.Font(family="Microsoft YaHei UI", size=10, weight='normal', slant='roman',
                                               underline=0, overstrike=0)
            self.TitleLabel = tkinter.Label(self.Form, anchor=tkinter.W, bg=self.bgColor, fg=self.fgColor,
                                            font=self.TitleFont, text=self.Title, width=100, height=1)
            self.TitleLabel.place(x=10, y=10, width=260, height=24)
            self.ProgressBar = tkinter.ttk.Progressbar(self.Form, length=200, mode='determinate', style="TProgressbar",
                                                       orient=tkinter.HORIZONTAL)
            self.ProgressBar.place(x=10, y=40, width=260, height=15)
            self.ProgressBar['maximum'] = 100
            self.ProgressBar['value'] = 0
            self.OKButton = tkinter.Button(self.Form, anchor=tkinter.CENTER, text='取消', width=100, height=1,
                                           bg=self.bgColor, fg=self.fgColor, command=self.submit)
